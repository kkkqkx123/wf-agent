...
| 前置观察 | 全部 `BEFORE_*` | 同步处理器专属（触发器闭合）：无处理器时定义只写出无人消费的审计事件，配置校验告警；门禁一律走审批 |

`BEFORE_*` 保留为正式的同步前置观察点（步骤开始前的定时探针与上下文快照），不移除：
每个都有对等领域事件可供触发器订阅（迭代/调用/节点起止），钩子侧的独特价值是同步屏障
（引擎等待处理器落定才进入该步骤）。无控制语义：只观测，不阻断、不改写。
