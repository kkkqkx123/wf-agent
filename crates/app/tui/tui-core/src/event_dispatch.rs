...
//! suppress or post-process events. The state type is generic so this crate
//! stays free of the application-shell `AppState`.
...
pub trait EventHandler<S>: Send + Sync {
    ...
    fn handle(&self, event: &EventType, state: &mut S) -> EventResult;
...
pub trait Middleware<S>: Send + Sync {
    ...
    fn before_dispatch(&self, _event: &EventType, _state: &mut S) -> bool {
        true
    }

    /// Called after event dispatch with the result.
    fn after_dispatch(&self, _event: &EventType, _state: &mut S, _result: &EventResult) {}
}

/// The event dispatcher: routes events through handlers with middleware.
pub struct EventDispatcher<S> {
    handlers: Vec<Box<dyn EventHandler<S>>>,
...
impl<S> EventDispatcher<S> {
    ...
    pub fn register_handler(&mut self, handler: Box<dyn EventHandler<S>>) {
    ...
    /// Register middleware.
    pub fn register_middleware(&mut self, mw: Box<dyn Middleware<S>>) {
        self.middleware.push(mw);
    }

    /// Dispatch an event through the middleware and handler chain.
    pub fn dispatch(&self, event: &EventType, state: &mut S) -> EventResult {
...
impl<S> Middleware<S> for LoggingMiddleware {
    fn after_dispatch(&self, event: &EventType, _state: &mut S, result: &EventResult) {
...
mod tests {
    ...
    /// Minimal test state so this low-level crate stays independent of the
    ...
    impl EventHandler<TestState> for TestHandler {
        fn handle(&self, _event: &EventType, _state: &mut TestState) -> EventResult {
            self.result
        }
        fn priority(&self) -> i32 {
            self.priority
    ...
    impl Middleware<TestState> for SuppressMiddleware {
        fn before_dispatch(&self, _event: &EventType, _state: &mut TestState) -> bool {
            false
        }
    }
    ...
    fn dispatch_returns_ignored_when_no_handlers() {
        let dispatcher = EventDispatcher::<TestState>::new();
    ...
    fn handler_priority_determines_order() {
        let mut dispatcher = EventDispatcher::<TestState>::new();
    ...
    fn disabled_handler_is_skipped() {
        ...
        impl EventHandler<TestState> for DisabledHandler {
            fn handle(&self, _event: &EventType, _state: &mut TestState) -> EventResult {
                EventResult::Consumed
            }
            fn is_enabled(&self) -> bool {
