...
impl ExecutionEventBus {
    ...
    pub fn publish(&self, event: &ExecutionEvent) {
        ...
        let type_handlers: Vec<Handler> = self
            ...
            .map(|list| list.clone())
            .unwrap_or_default();
        let all_handlers = wf_common::lock::read_ok(self.wildcard.read()).clone();
    ...
    pub fn clear(&self) {
        self.handlers.clear();
        wf_common::lock::write_ok(self.wildcard.write()).clear();
        wf_common::lock::write_ok(self.error_handlers.write()).clear();
    }
    ...
    fn handle_error(&self, message: &str) {
        ...
        let handlers = wf_common::lock::read_ok(self.error_handlers.read()).clone();
