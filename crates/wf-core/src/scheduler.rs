...
struct ScheduledTask {
    ...
    /// Optional callback for fire-and-forget tasks submitted via
    /// `submit_and_forget`.  When set, the background worker (or a
    /// `try_run_next` call) executes the callback and then calls
    /// `complete_task`.
...
pub type TaskCallback = Box<dyn FnOnce() -> futures::future::BoxFuture<'static, ()> + Send>;
...
impl TaskScheduler {
    ...
    pub fn schedule(
        ...
        let mut inner = wf_common::lock::lock_ok(self.inner.lock());
    ...
    /// Submit a fire-and-forget task with a callback.
    ///
    /// The task is scheduled with the given priority, timeout, and source
    /// identifier.  When the scheduler's background worker (or
    /// [`try_run_next`](Self::try_run_next)) picks it up, the callback is
    /// executed and the task is automatically completed.
    ///
    /// Returns an error only if the `task_id` duplicates an existing task.
    pub fn submit_and_forget(
        &self,
        task_id: String,
        source_id: String,
        ...
        let task = ScheduledTask {
            ...
            timeout,
            deadline,
            timeout_policy: TimeoutPolicy::Cancel,
        ...
        if inner.tasks.contains_key(&task_id) {
            return Err(CoreError::task_conflict(task_id));
        }
    ...
    /// Try to run the next pending fire-and-forget task.
    ///
    /// Returns `true` if a task was executed, `false` if no task is ready
    /// (either the pending queue is empty or `max_concurrent` has been
    /// reached).  The callback is spawned as a tokio task so the caller is
    /// not blocked; `complete_task` is called automatically when the
    /// callback finishes.
    ///
    /// This method is intended for callers that manage their own event loop
    /// and want to drain the scheduler inline.  The scheduler's own
    /// [`run_background`](Self::run_background) is usually more convenient.
    pub fn try_run_next(self: &Arc<Self>) -> bool {
        let (task_id, callback) = {
            let mut inner = wf_common::lock::lock_ok(self.inner.lock());
            ...
            // Find the next task with a callback (fire-and-forget tasks).
            let task_id = Self::next_fire_and_forget(&inner);
    ...
    /// Spawn a background worker that continuously polls and executes
    /// fire-and-forget tasks submitted via [`submit_and_forget`](Self::submit_and_forget).
    ///
    /// The worker respects `max_concurrent` and fair-scheduling settings.
    /// It runs until [`shutdown`](Self::shutdown) is called.
    pub fn run_background(self: &Arc<Self>) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // Keep draining while tasks are available and capacity permits.
                while this.try_run_next() {}
            }
        });
    }

    /// Find the next pending fire-and-forget task (one that has a callback).
    fn next_fire_and_forget(inner: &SchedulerInner) -> Option<String> {
        for priority in [
            TaskPriority::Critical,
            TaskPriority::High,
            TaskPriority::Normal,
    ...
    pub fn stats(&self) -> SchedulerStats {
        let inner = wf_common::lock::lock_ok(self.inner.lock());
