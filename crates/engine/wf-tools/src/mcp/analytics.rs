...
impl McpUsageAnalytics {
    ...
    pub fn record_execution(
        ...
        let mut stats = wf_common::lock::lock_ok(self.stats.lock());
        let entry = stats
            .entry(tool_id.clone())
    ...
    pub fn get_all_tool_stats(&self) -> Vec<ToolStats> {
        let mut values: Vec<ToolStats> = wf_common::lock::lock_ok(self.stats.lock())
            .values()
            .cloned()
            .collect();
    ...
    pub fn get_server_stats(&self, server_name: Option<&str>) -> HashMap<String, ServerStats> {
        ...
        for entry in wf_common::lock::lock_ok(self.stats.lock()).values() {
    ...
    pub fn generate_report(&self, limit: usize) -> AnalyticsReport {
        let all = wf_common::lock::lock_ok(self.stats.lock());
    ...
    pub fn get_execution_history(
        ...
        let tool_id = Self::tool_id(server_name, tool_name);
        let history = wf_common::lock::lock_ok(self.history.lock());
    ...
    pub fn clear(&self) {
        wf_common::lock::lock_ok(self.stats.lock()).clear();
        wf_common::lock::lock_ok(self.history.lock()).clear();
    }
}
