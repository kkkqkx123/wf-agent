...
export class ToolCalls {
  ...
  private async initialize(): Promise<void> {
    // Create the simplified tool_calls table
    await this.db.exec(`
  ...
  async start(name: string, parameters?: any): Promise<number> {
    ...
    const { id } = await stmt.get(name, parameters, started_at);
    return Number(id);
  }
  ...
  async success(id: number, result?: any): Promise<void> {
    ...
    await updateStmt.run(result, completed_at, duration_ms, id);
  }
  ...
  async record(
    ...
    const { id } = await stmt.get(name, parameters, result, error || null, status, started_at, completed_at, duration_ms);
    return Number(id);
  }
  ...
  async get(id: number): Promise<ToolCall | undefined> {
    ...
    const row = await stmt.get(id) as ToolCall | undefined;
  ...
  async getByName(name: string, limit?: number): Promise<ToolCall[]> {
    ...
    const rows = await stmt.all(name) as ToolCall[];
    return rows;
  }
  ...
  async getRecent(since: number, limit?: number): Promise<ToolCall[]> {
    ...
    const rows = await stmt.all(since) as ToolCall[];
    return rows;
  }
  ...
  async getStats(): Promise<ToolCallStats[]> {
    ...
    const rows = await stmt.all() as { name: string; total_calls: number; successful: number; failed: number; avg_duration_ms: number }[];
    return rows.map(row => ({
      name: row.name,
