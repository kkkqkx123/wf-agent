...
export function validateTriggeredSubgraphConnectivity(
  ...
  for (const node of graph.nodes.values()) {
    ...
    if (!reachableFromStart.has(node.id)) {
      errors.push(
        new ConfigurationValidationError(`Node (${node.id}) is not reachable from START_FROM_TRIGGER node`, {
          configType: "workflow",
          context: {
  ...
  for (const node of graph.nodes.values()) {
    ...
    if (!reachableFromNode.has(endNodeId)) {
      errors.push(
        new ConfigurationValidationError(`Node (${node.id}) cannot reach CONTINUE_FROM_TRIGGER node`, {
          configType: "workflow",
          context: {
