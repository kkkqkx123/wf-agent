...
/// (`a.b.c` creates nested objects). Targets with the `__` internal prefix
/// are rejected.
pub fn set_variable_path(
    ...
    if root.starts_with("__") {
        return Err(WorkflowError::VariableError(format!(
            "refusing to write internal variable '{}' through the public path writer",
            root
        )));
    }
...
mod tests {
    ...
    fn test_set_variable_path() {
        ...
        set_variable_path(&vars, "a.b.c", Value::from(1)).unwrap();
        assert_eq!(
            vars.get("a").unwrap().clone(),
            serde_json::json!({"b": {"c": 1}})
        );
        set_variable_path(&vars, "a.b.d", Value::from(2)).unwrap();
        assert_eq!(
            vars.get("a").unwrap().clone(),
            serde_json::json!({"b": {"c": 1, "d": 2}})
        );
        set_variable_path(&vars, "top", Value::from(3)).unwrap();
    ...
    fn test_variable_outputs_mapping() {
        ...
        apply_variable_outputs(&config, &child, &parent).unwrap();
        assert_eq!(
