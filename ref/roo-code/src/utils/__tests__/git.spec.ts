...
[remote "origin"]
 url = https://github.com/CoderInc/coder.git
...
`
		...
		expect(result).toEqual({
			repositoryUrl: "https://github.com/CoderInc/coder.git",
			repositoryName: "CoderInc/coder",
			defaultBranch: "main",
		})
...
[core]
	...
	bare = false
[remote "origin"]
	url = git@github.com:CoderInc/coder.git
	fetch = +refs/heads/*:refs/remotes/origin/*
[branch "main"]
	remote = origin
	merge = refs/heads/main
...
describe("convertGitUrlToHttps", () => {
	it("should leave HTTPS URLs unchanged", () => {
		const url = "https://github.com/CoderInc/coder.git"
		const converted = convertGitUrlToHttps(url)

		expect(converted).toBe("https://github.com/CoderInc/coder.git")
	})

	it("should convert SSH URLs to HTTPS format", () => {
		const url = "git@github.com:CoderInc/coder.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should convert SSH URLs with ssh:// prefix to HTTPS format", () => {
		const url = "ssh://git@github.com/CoderInc/coder.git"
		const converted = convertGitUrlToHttps(url)
	...
	it("should handle URLs without git@ prefix", () => {
		const url = "ssh://github.com/CoderInc/coder.git"
		const converted = convertGitUrlToHttps(url)
...
describe("sanitizeGitUrl", () => {
	it("should sanitize HTTPS URLs with credentials", () => {
		const url = "https://username:password@github.com/CoderInc/coder.git"
		const sanitized = sanitizeGitUrl(url)

		expect(sanitized).toBe("https://github.com/CoderInc/coder.git")
	})
	...
	it("should leave SSH URLs unchanged", () => {
		...
		expect(sanitized).toBe("git@github.com:CoderInc/coder.git")
	})
	...
	it("should leave SSH URLs with ssh:// prefix unchanged", () => {
		...
		expect(sanitized).toBe("ssh://git@github.com/CoderInc/coder.git")
	})

	it("should remove tokens from other URL formats", () => {
		const url = "https://oauth2:ghp_abcdef1234567890abcdef1234567890abcdef@github.com/CoderInc/coder.git"
		const sanitized = sanitizeGitUrl(url)
...
describe("extractRepositoryName", () => {
	it("should extract repository name from HTTPS URL", () => {
		...
		expect(repoName).toBe("CoderInc/coder")
	})

	it("should extract repository name from HTTPS URL without .git suffix", () => {
		const url = "https://github.com/CoderInc/coder"
		const repoName = extractRepositoryName(url)
