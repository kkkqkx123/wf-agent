...
/// Metadata key: true when a forced compression request was emitted without
/// a known context budget (no model window). The token limit is reported as
/// zero in that case so consumers never derive a ratio from it.
pub const KEY_BUDGET_UNKNOWN: &str = "budget_unknown";
...
pub const KEY_COMPRESSION_DEPTH: &str = "compression_depth";
/// Metadata key: terminal failure reason of a compression run.
...
/// Metadata key: model of a completed LLM call.
pub const KEY_MODEL: &str = "model";
/// Metadata key: prompt tokens reported by a completed LLM call.
pub const KEY_PROMPT_TOKENS: &str = "prompt_tokens";
/// Metadata key: completion tokens reported by a completed LLM call.
pub const KEY_COMPLETION_TOKENS: &str = "completion_tokens";
/// Metadata key: tools offered in an LLM request.
pub const KEY_TOOL_COUNT: &str = "tool_count";

/// Default token warning threshold percentage of the configured limit.
pub const DEFAULT_TOKEN_WARNING_THRESHOLD: u32 = 80;

/// Default number of recent messages kept visible alongside the summary
...
/// service) take over immediately; the `CONTEXT_COMPRESSION_REQUESTED` event
/// remains the audit / persistence / user-rule channel.
pub const COMPRESSION_SIGNAL_HOOK_TYPE: &str = wf_types::hook::CONTEXT_COMPRESSION_SIGNAL;
...
pub fn compression_request_hook_data(
    ...
    if request.forced && request.token_limit == 0 {
        data.insert(KEY_BUDGET_UNKNOWN.to_string(), serde_json::json!(true));
    }
...
/// execution identity that `base_event` already carries).
pub struct ContextCompressionCompleted<'a> {
    /// Target array name (workflow named context or agent conversation id).
    pub target_context_id: &'a str,
    ...
    /// Compressed message array for versioned write-back.
...
pub fn build_context_compression_completed_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    payload: &ContextCompressionCompleted<'_>,
) -> BaseEvent {
    let mut event = base_event(
    ...
    let mut pairs = vec![
        (
            KEY_TARGET_CONTEXT_ID,
            serde_json::json!(payload.target_context_id),
        ),
        (KEY_ARRAY_VERSION, serde_json::json!(payload.array_version)),
        (KEY_TOKENS_AFTER, serde_json::json!(payload.tokens_after)),
        (KEY_TAIL_KEEP, serde_json::json!(payload.tail_keep as u64)),
    ];
...
/// Build a CONTEXT_COMPRESSION_FAILED event.
///
/// Published when a compression run reaches its terminal failure state
/// (retries exhausted or a non-retryable error). Consumers clear their
/// in-flight compression record on this event; the emission guard stays so
/// the same array version does not loop.
pub fn build_context_compression_failed_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    ...
    event.metadata = Some(metadata(vec![
        ...
        (KEY_COMPRESSION_ATTEMPTS, serde_json::json!(attempts)),
        (KEY_COMPRESSION_ERROR, serde_json::json!(error)),
    ]));
    event
}
...
/// agent result is injected into the parent conversation. Carries the
/// operation (`replace` / `append`), the message array to apply, and the
/// anchor version the write-back was produced from: the conversation
/// consumer applies it only while the session is still at that version
/// (stale results are discarded, mirroring the compression write-back).
pub fn build_conversation_writeback_completed_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    target_context_id: &str,
    array_version: u64,
    operation: &str,
    messages: &[wf_types::message::Message],
) -> BaseEvent {
    let mut event = base_event(
    ...
    let mut pairs = vec![
        (KEY_TARGET_CONTEXT_ID, serde_json::json!(target_context_id)),
        (KEY_ARRAY_VERSION, serde_json::json!(array_version)),
        (KEY_WRITEBACK_OPERATION, serde_json::json!(operation)),
    ];
    if let Ok(value) = serde_json::to_value(messages) {
        pairs.push((KEY_MESSAGES, value));
    }
...
/// Build a LLM_REQUESTED event.
///
/// Emitted when an agent iteration issues a (non-streaming) LLM request,
/// before the gateway call begins. Carries profile id, message count and
/// tool count for online visibility.
pub fn build_llm_requested_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    profile_id: &str,
    message_count: usize,
    ...
    let mut event = base_event(EventType::LlmRequested, execution_id, agent_loop_id);
    event.metadata = Some(metadata(vec![
        (KEY_PROFILE_ID, serde_json::json!(profile_id)),
        (KEY_MESSAGE_COUNT, serde_json::json!(message_count)),
        (KEY_TOOL_COUNT, serde_json::json!(tool_count)),
    ]));
    event
}

/// Build a LLM_RESPONDED event.
///
/// Emitted when a (non-streaming) LLM request completed. Carries profile id,
/// model and reported token usage.
pub fn build_llm_responded_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    profile_id: &str,
    model: Option<&str>,
    ...
    let mut event = base_event(EventType::LlmResponded, execution_id, agent_loop_id);
    let mut pairs = vec![
        (KEY_PROFILE_ID, serde_json::json!(profile_id)),
        (KEY_PROMPT_TOKENS, serde_json::json!(prompt_tokens)),
        (KEY_COMPLETION_TOKENS, serde_json::json!(completion_tokens)),
    ];
    if let Some(model) = model {
        pairs.push((KEY_MODEL, serde_json::json!(model)));
    }
    event.metadata = Some(metadata(pairs));
    event
}

/// Build a LLM_FAILED event.
///
/// Emitted when a (non-streaming) LLM request failed. Carries the error
/// message and the profile id for correlation.
pub fn build_llm_failed_event(
    execution_id: &str,
    agent_loop_id: Option<&str>,
    error: &str,
    profile_id: &str,
) -> BaseEvent {
    let mut event = base_event(EventType::LlmFailed, execution_id, agent_loop_id);
    event.metadata = Some(metadata(vec![
        (KEY_STREAM_ERROR, serde_json::json!(error)),
        (KEY_PROFILE_ID, serde_json::json!(profile_id)),
    ]));
...
/// `tail_keep` is how many recent pre-existing messages stay visible
...
#[derive(Debug, Clone, PartialEq)]
pub struct ContextCompressionFailedMeta {
    pub target_context_id: String,
    pub array_version: u64,
    pub attempts: u32,
    pub error: String,
...
/// at that version.
#[derive(Debug, Clone, PartialEq)]
pub struct ConversationWritebackCompletedMeta {
    pub target_context_id: String,
    pub array_version: u64,
...
impl TryFrom<&BaseEvent> for ContextCompressionFailedMeta {
    type Error = TokenEventMetaError;

    fn try_from(event: &BaseEvent) -> Result<Self, Self::Error> {
        expect_type(event, EventType::ContextCompressionFailed)?;
        let meta = event_metadata(event)?;
        Ok(Self {
            target_context_id: get_string(meta, KEY_TARGET_CONTEXT_ID)?,
            array_version: get_u64(meta, KEY_ARRAY_VERSION)?,
            attempts: meta
                .get(KEY_COMPRESSION_ATTEMPTS)
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as u32,
            error: meta
                .get(KEY_COMPRESSION_ERROR)
                .and_then(|v| v.as_str())
                .unwrap_or_default()
                .to_string(),
        })
...
impl TryFrom<&BaseEvent> for ConversationWritebackCompletedMeta {
    type Error = TokenEventMetaError;

    fn try_from(event: &BaseEvent) -> Result<Self, Self::Error> {
        expect_type(event, EventType::ConversationWritebackCompleted)?;
        let meta = event_metadata(event)?;
        Ok(Self {
            target_context_id: get_string(meta, KEY_TARGET_CONTEXT_ID)?,
            array_version: get_u64(meta, KEY_ARRAY_VERSION)?,
            operation: get_string(meta, KEY_WRITEBACK_OPERATION)?,
            messages: get_messages(meta),
        })
    }
}
...
mod tests {
    ...
    fn test_compression_requested_event() {
        ...
        assert!(!forced_meta.contains_key(KEY_BUDGET_UNKNOWN));

        // Forced emission without a known budget reports zero limit plus
        // the unknown-budget marker instead of a fabricated ratio.
        let unknown = build_context_compression_requested_event(
            "exec-1",
            None,
            &ContextCompressionRequest {
                target_context_id: "chat",
        ...
        let unknown_meta = unknown.metadata.unwrap();
    ...
    fn test_compression_completed_event() {
        ...
        let no_summary = build_context_compression_completed_event(
            "exec-1",
            None,
            &ContextCompressionCompleted {
                target_context_id: "chat",
    ...
    fn test_parse_compression_completed_meta() {
        ...
        let failed =
            build_context_compression_failed_event("exec-1", None, "chat", 7, 3, "timed out");
        assert_eq!(failed.r#type, EventType::ContextCompressionFailed);
        let failed_meta = ContextCompressionFailedMeta::try_from(&failed).unwrap();
        assert_eq!(failed_meta.target_context_id, "chat");
        assert_eq!(failed_meta.array_version, 7);
        assert_eq!(failed_meta.attempts, 3);
        assert_eq!(failed_meta.error, "timed out");
    }

    #[test]
    fn test_writeback_completed_event_roundtrip() {
        let msg = wf_types::message::Message {
            id: "m1".to_string(),
            role: wf_types::message::MessageRole::Assistant,
            content: wf_types::message::MessageContentValue::Text("child result".to_string()),
            timestamp: 0,
            tool_call_id: None,
            tool_name: None,
            tool_calls: None,
        ...
        assert_eq!(event.r#type, EventType::ConversationWritebackCompleted);
        let meta = event.metadata.as_ref().unwrap();
        assert_eq!(
            meta[KEY_TARGET_CONTEXT_ID],
            serde_json::json!("conversation")
        );
        assert_eq!(meta[KEY_ARRAY_VERSION], serde_json::json!(3));
        assert_eq!(meta[KEY_WRITEBACK_OPERATION], serde_json::json!("append"));

        let parsed = ConversationWritebackCompletedMeta::try_from(&event).unwrap();
        assert_eq!(parsed.target_context_id, "conversation");
        assert_eq!(parsed.array_version, 3);
        assert_eq!(parsed.operation, WRITEBACK_OPERATION_APPEND);
        ...
        assert_eq!(
            ...
            TokenEventMetaError::TypeMismatch {
                expected: "CONVERSATION_WRITEBACK_COMPLETED",
                actual: "CONTEXT_COMPRESSION_REQUESTED",
            }
        );
    }
