//! Execution event types published on the `ExecutionEventBus`, aligned with
//! the TS `ExecutionEvent` union (packages/types/src/checkpoint/execution-events.ts
...
/// Union of all execution events, aligned with the TS `ExecutionEvent` union.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
