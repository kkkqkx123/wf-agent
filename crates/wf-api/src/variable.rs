...
impl VariableApi {
    ...
    pub async fn get(
        ...
        self.ctx
            ...
            .await?
            .ok_or_else(|| {
                ApiError::not_found("variable", &variable_key(execution_id, scope, name))
            })
    }
    ...
    pub async fn set(
        ...
        self.ctx.storage.variable.save(&record).await?;
        Ok(())
    }
    ...
    pub async fn define(
        ...
        self.set(name, scope, execution_id, value).await
    }
    ...
    pub async fn list_by_scope(&self, scope: &str) -> ApiResult<Vec<VariableStorageMetadata>> {
        self.ctx
            .storage
            .variable
    ...
    pub async fn export(&self, execution_id: &str) -> ApiResult<BTreeMap<String, Value>> {
        let records = self.list_by_execution(execution_id).await?;
    ...
    pub async fn history(
        ...
        if let Some(exec_id) = execution_id {
            if let Some(entity) = self.ctx.workflow_execution(exec_id) {
            ...
            if let Ok(Some(record)) = self.ctx.storage.workflow_execution.load(exec_id).await {
            ...
            if let Some(checkpoint_variables) = self.latest_checkpoint_variables(exec_id).await? {
    ...
    async fn latest_checkpoint_variables(
        ...
        let Some((bytes, _)) = self.ctx.checkpoint_store.load(&latest.id).await? else {
            return Ok(None);
        };
...
mod tests {
    ...
    async fn variable_crud_and_scoping() {
        let ctx = make_ctx();
        let api = VariableApi::new(ctx);
        api.set("count", "local", Some("exec-1"), serde_json::json!(1))
            .await
            .unwrap();
        api.set("count", "local", Some("exec-1"), serde_json::json!(2))
            .await
            .unwrap();

        let loaded = api.get("count", "local", Some("exec-1")).await.unwrap();
        assert_eq!(loaded.value, serde_json::json!(2));

        // Same name in a different scope is independent.
        let err = api.get("count", "other", Some("exec-1")).await.unwrap_err();
        assert!(matches!(err, ApiError::NotFound { .. }));

        assert!(api.delete("count", "local", Some("exec-1")).await.unwrap());
        assert!(!api.delete("count", "local", Some("exec-1")).await.unwrap());
    }
    ...
    async fn define_rejects_existing() {
        let ctx = make_ctx();
        let api = VariableApi::new(ctx);
        api.define("x", "default", None, serde_json::json!(1))
            .await
            .unwrap();
        let err = api
            .define("x", "default", None, serde_json::json!(2))
            .await
            .unwrap_err();
        assert!(matches!(
            err,
    ...
    async fn export_lists_execution_variables() {
        let ctx = make_ctx();
        let api = VariableApi::new(ctx);
        api.set("a", "local", Some("exec-2"), serde_json::json!(1))
            .await
            .unwrap();
        api.set("b", "local", Some("exec-2"), serde_json::json!("two"))
            .await
            .unwrap();
        api.set("c", "other", Some("exec-2"), serde_json::json!(3))
            .await
            .unwrap();

        let exported = api.export("exec-2").await.unwrap();
        assert_eq!(exported.len(), 3);
        assert_eq!(exported.get("b"), Some(&serde_json::json!("two")));

        let by_scope = api.list_by_scope("local").await.unwrap();
    ...
    async fn history_collects_sources() {
        ...
        let history = api
            .history("shared", "local", Some("exec-3"))
            .await
            .unwrap();
