...
/// Validate a hook type against the unified hook type registry
/// (`wf_types::hook`). The registry is the single source of truth shared by
...
pub fn validate_hook_type(hook_type: &str, field_name: &str) -> ConfigResult<()> {
    if is_known_hook_type(hook_type) {
        ...
        Err(ConfigError::Validation(format!(
            "{field_name} references unknown hook type '{hook_type}'"
        )))
    }
}
