...
pub trait VfsProvider: Send + Sync {
    ...
    /// The path policy backing this VFS, when available. Execution-time
...
impl DefaultStrategyResolver {
    ...
    fn register_default_strategies(&mut self) {
        ...
        use crate::strategy::shell::proot::ProotStrategy;
        self.shell_strategies
            .insert("proot".to_string(), Arc::new(ProotStrategy));
        use crate::strategy::shell::job_object::JobObjectStrategy;
        self.shell_strategies
            .insert("job-object".to_string(), Arc::new(JobObjectStrategy));
...
mod tests {
    ...
    fn test_resolve_chain_accepts_analysis_before_execution() {
        ...
        let chain = resolver
            .resolve_chain(
                "shell",
                &["static-analyzer".to_string(), "os-hook".to_string()],
            )
            .unwrap();
        let ids: Vec<&str> = chain.iter().map(|s| s.id()).collect();
