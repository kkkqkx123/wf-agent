...
/// (`"runtime" | "workflow" | "node" | "agent" | "global" | "default"`).
...
/// A single checkpoint configuration layer (source + config).
#[derive(Debug, Clone)]
...
impl CheckpointConfigResolver {
    ...
    /// the trigger-based description.
    pub fn build_description(
        policy: &UnifiedCheckpointPolicy,
        trigger: Option<&CheckpointTiming>,
        ...
        match (trigger, iteration) {
            (Some(CheckpointTiming::OnError), _) => "Error checkpoint".to_string(),
    ...
    pub fn should_checkpoint_before_node(triggers: &[CheckpointTiming]) -> bool {
        triggers.contains(&CheckpointTiming::BeforeExecute)
    }

    pub fn should_checkpoint_after_node(triggers: &[CheckpointTiming]) -> bool {
        triggers.contains(&CheckpointTiming::AfterExecute)
    }

    pub fn should_checkpoint_on_error(triggers: &[CheckpointTiming]) -> bool {
        triggers.contains(&CheckpointTiming::OnError)
    }

    pub fn should_checkpoint_on_pause(triggers: &[CheckpointTiming]) -> bool {
...
mod tests {
    ...
    fn policy(enabled: bool, triggers: Vec<CheckpointTiming>) -> UnifiedCheckpointPolicy {
        UnifiedCheckpointPolicy {
            enabled,
            triggers,
            content: None,
    ...
    fn test_should_checkpoint() {
        let triggers = vec![CheckpointTiming::BeforeExecute, CheckpointTiming::OnError];
    ...
    fn resolve_first_wins_highest_priority() {
        let layers = vec![
            ...
            CheckpointConfigLayer::runtime(policy(true, vec![CheckpointTiming::AfterExecute])),
        ];
    ...
    fn resolve_lower_layer_fields_fill_gaps() {
        ...
        let layers = vec![
            CheckpointConfigLayer::global(policy(true, vec![CheckpointTiming::OnError])),
    ...
    fn build_description_variants() {
        ...
        assert_eq!(
            CheckpointConfigResolver::build_description(
                &policy(true, Vec::new()),
                Some(&CheckpointTiming::IterationEnd),
                Some(7),
            ),
