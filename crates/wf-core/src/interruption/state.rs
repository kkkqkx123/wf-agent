...
impl InterruptionState {
    ...
    pub fn set_event_bus(&self, event_bus: Arc<EventBus>) {
        *wf_common::lock::write_ok(self.event_bus.write()) = Some(event_bus);
    }

    /// The event bus this interruption publishes lifecycle events to.
    pub fn event_bus(&self) -> Option<Arc<EventBus>> {
        wf_common::lock::read_ok(self.event_bus.read()).clone()
    }
    ...
    pub fn pause(&self) -> CoreResult<()> {
        ...
        // Pause is a suspension, not a cancellation: it must not abort an
        // in-flight iteration or cancel the execution token. Only `stop`
        // cancels, so a paused loop can be resumed without losing state.
    ...
    fn emit_event(&self, event_type: EventType) {
        let bus = wf_common::lock::read_ok(self.event_bus.read()).clone();
        if let Some(bus) = bus {
            let event = BaseEvent {
                id: wf_types::Id::new(),
                r#type: event_type,
...
mod tests {
    ...
    fn test_pause_signal() {
        ...
        assert!(state.is_interrupted());
        // Pause is a suspension, not a cancellation.
    ...
    fn test_cancellation_token_propagation() {
        ...
        // Only stop cancels the execution token; pause must not.
        state.pause().unwrap();
        assert!(!token.is_cancelled());
    ...
    #[test]
    fn test_resume_keeps_stop_token_cancelled() {
        let state = InterruptionState::new();
        let token = state.get_cancellation_token();
