...
use crate::common::diff::unified_diff_text;
...
impl FileCheckpointManager {
    ...
    fn check_workspace_root_binding(
        ...
        match storage
            .load_metadata(crate::metadata::keys::WORKSPACE_ROOT_KEY)
            .map_err(map_layertwine_error)?
        {
            ...
            Some(_) => Ok(()),
            None => storage
                .store_metadata(crate::metadata::keys::WORKSPACE_ROOT_KEY, &normalized)
                .map_err(map_layertwine_error),
        }
    }
    ...
    pub fn create_workspace_checkpoint(
        ...
        self.storage_ref()?
            .store_metadata(
                &crate::metadata::keys::empty_dirs_key(&checkpoint.id),
