...
async fn generate_parses_openai_response() {
    ...
    let result = client.generate(&request("hi")).await.expect("generate");
...
async fn generate_stream_propagates_http_errors() {
    ...
    let err = match client.generate_stream(&request("hi")).await {
...
async fn count_tokens_falls_back_to_local_estimate() {
    ...
    let result = client.count_tokens(&req).await.expect("count");
    assert_eq!(
        server.call_count(),
        0,
