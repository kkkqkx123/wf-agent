...
impl ToolMetricsCollector {
    ...
    /// Record a `general` inner invocation (grouped by the inner tool name).
    pub fn record_general_invoke(&self, inner_tool: &str, success: bool, duration_ms: f64) {
        self.inner.increment_counter(
            tool_metrics::GENERAL_INVOKE_COUNT,
            labels(&[
        ...
        self.inner.observe_histogram(
            tool_metrics::GENERAL_INVOKE_DURATION,
            duration_ms,
            labels(&[("inner_tool", inner_tool)]),
        );
    }

    /// Record a first-time tool discovery (method: `activation` | `general`).
    pub fn record_discovery(&self, tool_name: &str, method: &str) {
        self.inner.increment_counter(
            tool_metrics::DISCOVERY_COUNT,
            labels(&[("tool", tool_name), ("method", method)]),
        );
    }

    /// Record a formal TOOL_VISIBILITY unblock (activation).
    pub fn record_activation(&self, tool_name: &str) {
        self.inner.increment_counter(
            tool_metrics::ACTIVATION_COUNT,
            labels(&[("tool", tool_name)]),
        );
        self.record_discovery(tool_name, "activation");
    }
    ...
    pub async fn usage_stats_with_history(&self) -> ToolUsageStats {
        ...
        .await;
        let errors = crate::collectors::counter_total_with_history(
            &self.inner,
            tool_metrics::ERROR_COUNT,
            &std::collections::HashMap::new(),
        )
...
mod tests {
    ...
    #[test]
    fn records_general_invoke_and_discovery() {
        let c = collector();
        c.record_general_invoke("web_search", true, 42.0);
        c.record_general_invoke("web_search", false, 7.0);
        c.record_activation("write_file");
        c.record_discovery("edit_file", "general");

        let invokes =
            crate::collectors::counter_total(&c.inner, tool_metrics::GENERAL_INVOKE_COUNT);
        assert_eq!(invokes, 2.0);
        let duration = crate::collectors::latest(&c.inner, tool_metrics::GENERAL_INVOKE_DURATION);
        assert_eq!(duration.as_ref().unwrap().count, 2);

        let activation = crate::collectors::counter_total(&c.inner, tool_metrics::ACTIVATION_COUNT);
        assert_eq!(activation, 1.0);
        let discovery = crate::collectors::counter_total(&c.inner, tool_metrics::DISCOVERY_COUNT);
        // activation records one discovery + the explicit general one.
