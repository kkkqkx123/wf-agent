...
export class SessionPicker implements Component, CoreModal {
  readonly component: Component = this;
  private selectedIndex = 0;

  constructor(
    private sessions: SessionItem[],
    private onSelect?: (session: SessionItem | null) => void,
  ) {}
  ...
  static pick(
    ...
    return new Promise<SessionItem | null>((resolve) => {
      const modal = new SessionPicker(sessions, resolve);
  ...
  handleKey(key: string): ModalAction {
    ...
    if (kb.matches(key, "tui.select.confirm") || key === "\r") {
      const selected = this.sessions[this.selectedIndex] ?? null;
  ...
  render(width: number): string[] {
    ...
    for (let i = start; i < end; i++) {
      ...
      const timeStr = session.timestamp
        ...
        : "";
      const displayText = timeStr ? `${label} (${timeStr})` : label;
      const truncated = truncateToWidth(displayText, innerWidth - visibleWidth(prefix), "");
