...
impl ShellSession {
    ...
    fn append_output(buf: &Arc<Mutex<OutputBuffer>>, chunk: &str) {
        wf_common::lock::lock_ok(buf.lock()).append(chunk);
    }
    ...
    pub fn status(&self) -> (String, Option<i32>) {
        {
            let exit = wf_common::lock::lock_ok(self.exit.lock());
        ...
        if status == "exited" {
            ...
            let mut exit = wf_common::lock::lock_ok(self.exit.lock());
            if exit.is_none() {
                *exit = Some(ExitState {
                    status: status.clone(),
                    code,
    ...
    pub fn wait_for_exit(&self, timeout: Duration) -> (Option<i32>, bool) {
        ...
        while exit.is_none() {
            ...
            let (guard, timed_out) =
                wait_timeout_ok(self.exit_cv.wait_timeout(exit, deadline - now));
            exit = guard;
