...
impl CheckpointErrorHandler {
    ...
    ///
    /// when the config (or the `failOnCheckpointError` field) is
    /// absent, checkpoint write failures are treated as visible failures
    /// (`fail_on_checkpoint_error` defaults to `true`) instead of being
    /// silently swallowed. Callers that want checkpoint errors to never
    /// disturb the execution must opt out explicitly.
    ...
    pub fn from_config(config: Option<&CheckpointErrorHandlingConfig>) -> Self {
        ...
        if let Some(config) = config {
            handler.fail_on_checkpoint_error = config.fail_on_checkpoint_error.unwrap_or(true);
...
mod tests {
    ...
    #[test]
    fn policy_without_error_handling_defaults_to_visible_failures() {
        // an absent error_handling config must not silently swallow
        ...
        let handler = CheckpointErrorHandler::from_policy(&policy);
        assert!(
            handler.fail_on_checkpoint_error(),
            "defaults to fail-fast so write failures are visible"
        );
        assert!(handler.decide(&context(0), &error()).should_rethrow);
    }
