/**
 * BaseFileCheckpointResourceAPI - Shared base class for file checkpoint resource management
 *
 * Provides a unified base implementation for file checkpoint CRUD operations.
 ...
 * Entity-specific subclasses provide their own filter types and entity-scoped method names.
 */
export class BaseFileCheckpointResourceAPI<
  TFilter extends BaseFileCheckpointFilter = BaseFileCheckpointFilter,
> extends SimplifiedCrudResourceAPI<FileCheckpointMetadata, string, TFilter> {
  protected deps: APIDependencyManager;
   ...
   */
  protected getManager(): FileCheckpointManager {
  ...
  /**
   * Create a file checkpoint for the given entity
   *
   * @param entityId - Entity ID (workflow execution ID or agent loop ID)
   * @returns The created checkpoint ID and metadata
   */
  async createFileCheckpoint(entityId: string): Promise<FileCheckpointCreateResult> {
    const manager = this.getManager();
    return manager.createCheckpoint(entityId);
  }

  /**
   * Restore workspace files from a file checkpoint
   *
   * @param entityId - Entity ID associated with the checkpoint
  ...
  async restoreFileCheckpoint(
    ...
    const manager = this.getManager();
    return manager.restoreCheckpoint(entityId, checkpointId);
  }
  ...
  /**
   * List file checkpoints for a specific entity
   *
   * @param entityId - Entity ID to list checkpoints for
  ...
  async listByEntity(
    ...
    const manager = this.getManager();
    const storage = manager.getStorage();
    const result = await storage.listByEntity(entityId, options);
    return result.map(r => ({
      checkpointId: r.id,
      metadata: r.metadata,
    }));
  ...
  /**
   * Delete all file checkpoints for a specific entity
   *
  ...
  async deleteByEntity(entityId: string, keepLatest?: number): Promise<number> {
    const manager = this.getManager();
    const storage = manager.getStorage();
    return storage.deleteByEntity(entityId, keepLatest);
  }
