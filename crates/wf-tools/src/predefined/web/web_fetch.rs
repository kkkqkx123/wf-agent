    ...
    parameters: &[
        ...
        ToolParameter {
            ...
            constraints: Some(&ToolParameterConstraint {
                enum_values: Some(&["text", "markdown", "html"]),
                pattern: None,
                min_length: None,
                max_length: None,
                minimum: None,
...
pub fn web_fetch_handler(config: &WebToolConfig) -> StatelessAsyncHandler {
    ...
    // Build the client once per handler: `reqwest::Client` is cheap to clone
    // (Arc-shared) and reuses its connection pool across requests, whereas a
    // per-request client would rebuild the pool every call.
