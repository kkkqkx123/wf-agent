...
impl TerminalSession {
    ...
    fn sync_status(&self) -> SessionStatus {
        let current = wf_common::lock::lock_ok(self.current.lock());
        let status = wf_common::lock::lock_ok(self.status.lock());
    ...
    pub fn last_exit_code(&self) -> Option<i32> {
        *wf_common::lock::lock_ok(self.last_exit_code.lock())
    }

    /// Terminal mode of the session.
    pub fn mode(&self) -> SessionMode {
        *wf_common::lock::lock_ok(self.mode.lock())
    }
    ...
    pub(crate) fn terminate(&self, graceful: bool) -> ShellResult<()> {
        ...
        let result = match wf_common::lock::lock_ok(self.current.lock()).clone() {
    ...
    pub(crate) fn output_start(&self) -> usize {
        wf_common::lock::lock_ok(self.output.lock()).written()
    }

    pub(crate) fn tail_output(&self, start: usize) -> String {
        wf_common::lock::lock_ok(self.output.lock()).tail_from(start)
    }

    /// Incremental read: output since the last call, advancing the cursor.
    pub fn read_new_output(&self) -> String {
        wf_common::lock::lock_ok(self.output.lock()).read_new()
    }
    ...
    pub fn snapshot(&self) -> Value {
        ...
        serde_json::json!({
            ...
            "mode": self.mode_str(),
            "task_id": wf_common::lock::lock_ok(self.task_id.lock()).clone(),
            "cwd": self.cwd.as_ref().map(|p| p.to_string_lossy().to_string()),
            "created_at": self.created_at,
            "running_seconds": running_seconds,
            "output": wf_common::lock::lock_ok(self.output.lock()).snapshot(),
        })
    }
    ...
    pub(crate) fn dispatch_created(&self, reused: bool) {
        if self.events_enabled {
            if let Some(dispatcher) = &self.event_sink {
                let task_id = wf_common::lock::lock_ok(self.task_id.lock()).clone();
    ...
    pub(crate) fn finalize(
        ...
        let mut current = wf_common::lock::lock_ok(self.current.lock());
        let mut status = wf_common::lock::lock_ok(self.status.lock());
        if *status != SessionStatus::Busy {
            return;
        }
        *current = None;
        *status = SessionStatus::Idle;
        *wf_common::lock::lock_ok(self.last_exit_code.lock()) = exit_code;
        *wf_common::lock::lock_ok(self.last_active_at.lock()) = wf_common::time::now();
        drop(current);
        drop(status);
