/**
 * Graph Navigator
 * Used to navigate through the nodes of a graph during execution, determining the next node to be processed.
 *
 * Design Principles:
 ...
 * - Stateful Definition: Holds an immutable workflow definition (GraphData), which is managed by the parent container (ThreadContext) throughout its lifecycle.
 * - Pure Functions: All methods are pure functions and do not have any side effects.
 *
 * Creation Method: Instances of GraphNavigator are created and cached uniformly by ThreadContext, initialized using the graph object within the Thread.
 */

import type { ID, NodeType, Condition, Edge, Graph } from "@wf-agent/types";
...
export class GraphNavigator {
  ...
  /**
   * Select the next node based on the Thread context
   * @param currentNodeId: The ID of the current node
   * @param thread: A Thread instance that provides context information such as variables, inputs, and outputs
  ...
  async selectNextNodeWithContext(
    ...
    const satisfiedEdges = await this.filterEdgesWithContext(outgoingEdges, thread);
  ...
  /**
   * Filter edges that meet the conditions based on the Thread context
  ...
  private async filterEdgesWithContext(
    ...
    const results = await Promise.all(
      edges.map(async edge => ({
        edge,
        satisfied: await this.evaluateEdgeConditionWithContext(edge, thread),
      })),
    );
  ...
  /**
   * Evaluate the condition of an edge based on the Thread context
  ...
  private async evaluateEdgeConditionWithContext(
    ...
    const context = {
      ...
      input: thread.input as Record<string, unknown>,
      output: thread.output as Record<string, unknown>,
    };
