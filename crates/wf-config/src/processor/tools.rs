...
//!
//! Mirrors the TS `shared/config/processors/tools` (glob.ts, list-files.ts,
//! read-file.ts). The Rust form uses `ConfigResult` instead of the TS
//! `{valid, errors}` pair, and the identity `exportXxx` helpers are omitted.
