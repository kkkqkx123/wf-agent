...
async fn build_snapshot(
    ...
    let variables = state_snapshot
        ...
        .collect();
    snapshot.status = crate::execution_state::status_str(&status).to_string();
...
pub async fn restore(ctx: &ApiContext, agent_loop_id: &str, checkpoint_id: &str) -> ApiResult<Checkpoint> {
    let checkpoint = ctx
        .storage
        .checkpoint
        .load(checkpoint_id)
...
mod tests {
    ...
    async fn create_list_and_statistics() {
        ...
        let cp1 = create(&ctx, "loop-c", CheckpointType::Full, Some(vec!["initial".into()]))
            .await
            .unwrap();
