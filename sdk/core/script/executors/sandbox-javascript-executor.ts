/**
 * Sandbox JavaScript Executor
 *
 * Executes JavaScript code through the sandbox runtime with policy enforcement.
...
const logger = createContextualLogger({ component: "SandboxJavaScriptExecutor" });

export class SandboxJavaScriptExecutor extends BaseExecutor {
    ...
    try {
      ...
      return {
        ...
        scriptName: options.sandboxConfig?.profile
          ? `sandbox-javascript:${options.sandboxConfig.profile}`
          : "sandbox-javascript",
      };
    } catch (error) {
      return {
