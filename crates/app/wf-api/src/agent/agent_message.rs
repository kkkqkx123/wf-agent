...
use crate::entity::message::MessageStats;
use crate::entity::message::{estimate_tokens, message_text, role_name};
use crate::infra::context::ApiContext;
use crate::infra::error::ApiResult;

/// Per-agent-loop message statistics.
...
pub async fn global_stats(ctx: &ApiContext) -> ApiResult<MessageStats> {
    crate::entity::message::stats(ctx).await
}
...
///
/// Note: deliberately not named `normalize_history` — the entity-level
/// `message::normalize_history` is a read-only dedup that returns the
/// messages, while this function deletes duplicates from storage.
pub async fn dedupe_and_delete(ctx: &ApiContext, agent_loop_id: &str) -> ApiResult<usize> {
    let messages = ctx
        .storage
        .message
        .list_by_agent_loop(agent_loop_id, None)
...
mod tests {
    ...
    async fn recent_search_stats_and_count() {
        ...
        let history = conversation_history(&ctx, "loop-m", Some(10))
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn dedupe_and_delete_keeps_latest() {
        let ctx = make_ctx();
        ...
        let removed = dedupe_and_delete(&ctx, "loop-n").await.unwrap();
        assert!(removed >= 1);
        let remaining = ctx
            .storage
            .message
