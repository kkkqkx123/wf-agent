...
export class RejectionMessageBuilder {
  ...
  buildRejectionMessage(toolId: string, reason?: string): string {
    ...
    const globalTemplate = this.config.globalDefaultTemplate 
      || DEFAULT_REJECTION_CONFIG.globalDefaultTemplate!;
  ...
  buildUserMessageHint(
    ...
    const template = this.config.userMessageHintTemplate 
      || DEFAULT_REJECTION_CONFIG.userMessageHintTemplate!;
