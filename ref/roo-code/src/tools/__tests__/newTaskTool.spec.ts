...
describe("newTaskTool", () => {
	...
	describe("VSCode setting: newTaskRequireTodos", () => {
		...
		it("should check VSCode setting with Package.name configuration key", async () => {
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("coder-roo")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})

		it("should use current Package.name value (coder-nightly) when accessing VSCode configuration", async () => {
			...
			const pkg = await import("../../../shared/package")
				; (pkg.Package as any).name = "coder-nightly"
			...
			expect(mockGetConfiguration).toHaveBeenCalledWith("coder-nightly")
			expect(mockGet).toHaveBeenCalledWith("newTaskRequireTodos", false)
		})
	})
