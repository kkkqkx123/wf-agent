...
import { SqliteTaskStorage } from "../sqlite-task-storage.js";
...
describe("SqliteTaskStorage", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "sqlite-task-test-"));
  ...
  describe("save / load", () => {
    ...
    it("should overwrite existing task", async () => {
      ...
      await storage.save(taskId, data1, createMetadata());
      await storage.save(taskId, data2, createMetadata());
