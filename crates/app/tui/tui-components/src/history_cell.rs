...
impl HistoryCell for AssistantMessageCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        for line in self.text.lines() {
            if line.width() <= w {
                let spans = tui_style::motion::shimmer_text(line, self.motion_mode);
                out.push(Line::from(spans));
            } else {
                ...
                for wrapped_line in word_wrap_plain(line, w) {
                    let plain_text: String = wrapped_line
                        ...
                        .map(|s| s.content.as_ref())
                        .collect();
                    let spans = tui_style::motion::shimmer_text(&plain_text, self.motion_mode);
                    out.push(Line::from(spans));
                }
            }
        }
    ...
    fn animation_tick(&self) -> Option<u64> {
        if self.motion_mode.should_animate() {
            ...
            Some(tui_clock::clock::anim_bucket(tui_clock::clock::now_ms()))
        } else {
            None
        }
    }
