...
/// Sort `(id, stats)` pairs by the stats' `total` field descending with a
/// stable id tie-break, truncated to `limit`. Shared by the top-N helpers.
fn rank_by_total<T: UsageTotal>(
    entries: impl Iterator<Item = (String, T)>,
    limit: usize,
) -> Vec<(String, T)> {
    let mut ranked: Vec<(String, T)> = entries.collect();
    ranked.sort_by(|a, b| b.1.total().cmp(&a.1.total()).then_with(|| a.0.cmp(&b.0)));
    ranked.truncate(limit);
    ranked
}

/// Minimal `total` accessor shared by the usage stat structs.
trait UsageTotal {
    fn total(&self) -> u64;
}

impl UsageTotal for WorkflowUsageStats {
    fn total(&self) -> u64 {
        self.total
    }
}

impl UsageTotal for AgentUsageStats {
    fn total(&self) -> u64 {
        self.total
    }
}
...
mod tests {
    ...
    fn seeded_registry() -> MetricsRegistry {
        ...
        registry
            .workflow()
            .record_execution_complete("wf-2", None, false, 20.0, None);
        ...
        registry.agent().record_execution_start("profile-a");
        registry
            .agent()
            .record_execution_complete("profile-a", true, 10.0);
        registry.agent().record_execution_start("profile-b");
        registry
            .agent()
            .record_execution_complete("profile-b", true, 10.0);
