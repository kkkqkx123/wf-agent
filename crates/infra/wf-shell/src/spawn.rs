...
//! semantics.
//!
//! **Env inheritance rule**: every spawned shell inherits the parent
...
//!
//! When a sandbox policy is attached, the shell runs through the shared
//! `wf-sandbox` execution gateway instead: the inherited environment is
//! cleared (minimal PATH/HOME/TMPDIR restored) before the explicit `env`
//! overlay is applied, so host secrets do not leak into the child.
...
use wf_sandbox::cmd::{self, ApplyOptions};
...
///
/// When `policy` is `Some`, the shared sandbox gateway hardens the spawn
/// (seccomp + rlimits + env clearing).
pub fn build_shell_command(
    ...
    if let Some(policy) = policy {
        ...
        .map_err(|e| ShellError::ExecutionError(format!("Failed to sandbox command: {e}")))?;
    }
    Ok(cmd)
}
...
/// overlay merged on top (see the crate-level env inheritance note); when
/// `policy` is `Some` the environment is cleared first and `env` is the only
/// injection point.
pub fn run_shell_blocking(
    ...
    let mut cmd = build_shell_command(shell_type, command, cwd, policy)?;
...
mod tests {
    ...
    fn echo_policy() -> Option<SandboxPolicy> {
        // Default strict policy so sandboxed tests exercise the gateway.
        Some(wf_sandbox::default_policy::default_sandbox_policy().clone())
    }
    ...
    #[test]
    fn test_run_shell_blocking_echo_sandboxed() {
        let output = run_shell_blocking(
            None,
            "echo hello-sandboxed",
            None,
            None,
            None,
            Duration::from_secs(10),
        ...
        assert!(
            ...
            "sandboxed echo should work: {:?}",
            String::from_utf8_lossy(&output.stderr)
        );
        assert!(String::from_utf8_lossy(&output.stdout).contains("hello-sandboxed"));
    }
    ...
    #[test]
    fn test_run_shell_blocking_sandboxed_env_is_cleared() {
        std::env::set_var("WF_LEAK_SECRET", "top-secret");
        let mut env = HashMap::new();
        env.insert("WF_TEST_OVERLAY".to_string(), "overlay-value".to_string());
        let output = run_shell_blocking(
            None,
            "echo ${WF_LEAK_SECRET:-none} $WF_TEST_OVERLAY",
            None,
            Some(&env),
            None,
            Duration::from_secs(10),
        ...
        assert!(
            ...
            "sandboxed env must be cleared: {stdout}"
        );
        assert!(
            stdout.contains("overlay-value"),
            "explicit overlay must survive env clearing: {stdout}"
        );
        std::env::remove_var("WF_LEAK_SECRET");
    }
