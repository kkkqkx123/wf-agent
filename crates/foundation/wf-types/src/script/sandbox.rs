...
pub struct ProcessPolicy {
    ...
    /// When `true`, the seccomp filter uses allow-list semantics (default
    /// deny, only the syscalls required for basic execution plus the
    /// policy-derived ones are permitted). When `false`/`None` the filter is
    /// a deny-list (default allow) — Docker-style, suitable for arbitrary
    /// shell commands. Allow-list mode is stricter but breaks programs that
    /// need exotic syscalls, so it is opt-in.
    pub allowlist_syscalls: Option<bool>,
}
