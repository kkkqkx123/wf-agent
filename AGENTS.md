...
Top level contains `apps/`, `Cargo.toml`, `rust-toolchain.toml`, `crates/`. The JS side uses npm workspaces rooted at `apps/package.json` (members: `web-app`, `web-app-preview`); `tools/openapi-codegen` is a standalone npm package.

`apps/` contains `web-app` (main frontend) and `web-app-preview` (a fixture-backed mirror generated by `scripts/sync-frontend-preview.sh`).
