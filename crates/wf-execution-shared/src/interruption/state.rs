...
impl InterruptionState {
    ...
    pub fn resume(&self) -> ExecutionSharedResult<()> {
        self.ensure_not_disposed()?;
        self.tx.send(InterruptionSignal::Active).map_err(|_| {
            ExecutionSharedError::InterruptionError("failed to send resume signal".to_string())
        })?;
    ...
    fn ensure_not_disposed(&self) -> ExecutionSharedResult<()> {
        if self.is_disposed() {
