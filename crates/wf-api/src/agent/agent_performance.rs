...
use crate::agent::agent_loop_registry::ExecutionTimelineEntry;
use crate::infra::context::ApiContext;
use crate::infra::error::ApiResult;
use crate::infra::util::round2;

/// Timing of one agent iteration.
...
/// Performance profile of one agent loop execution.
#[derive(Debug, Clone, Serialize)]
...
/// Iteration-level duration comparison.
#[derive(Debug, Clone, Serialize)]
...
pub async fn analyze_performance(
    ...
    let details = crate::agent::agent_loop_registry::iteration_history(ctx, execution_id).await?;
    let Some(summary) = crate::agent::agent_loop_registry::summary(ctx, execution_id).await? else {
        return Ok(None);
    };
...
pub async fn execution_timeline(
    ...
    crate::agent::agent_loop_registry::execution_timeline(ctx, execution_id).await
}
...
mod tests {
    ...
    async fn analyze_performance_builds_profile() {
        ...
        let profile = analyze_performance(&ctx, "loop-perf")
            .await
            .unwrap()
            .unwrap();
        ...
        assert!(analyze_performance(&ctx, "missing")
            .await
            .unwrap()
            .is_none());
    }
