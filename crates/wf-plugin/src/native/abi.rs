...
pub struct ContributionRegistrarC {
    ...
    pub register_hook_handler:
        Option<extern "C" fn(ctx: *mut std::ffi::c_void, hook_type: *const c_char) -> i32>,
...
pub fn load_manifest(
    ...
    if result != 0 {
        return Err(crate::error::PluginError::NativeError(
            "wf_plugin_get_manifest failed (size query)".into(),
        ));
    }
    if len == 0 {
        return Err(crate::error::PluginError::NativeError(
            "wf_plugin_get_manifest returned zero size".into(),
        ));
    }
    ...
    if result != 0 {
        return Err(crate::error::PluginError::NativeError(
            "wf_plugin_get_manifest failed (fill)".into(),
        ));
    }
...
pub fn load_abi_info(lib: &libloading::Library) -> PluginResult<PluginAbiResult> {
    let get_manifest: libloading::Symbol<extern "C" fn(*mut u8, *mut usize) -> i32> = unsafe {
        lib.get(b"wf_plugin_get_manifest").map_err(|e| {
            crate::error::PluginError::NativeError(format!("missing wf_plugin_get_manifest: {}", e))
        })?
    };
...
fn check_abi_version(lib: &libloading::Library) -> PluginResult<()> {
    let version_fn: Option<libloading::Symbol<extern "C" fn() -> u32>> =
        unsafe { lib.get(b"wf_plugin_abi_version").ok() };
    match version_fn {
        Some(sym) => {
            let version = sym();
            if version != WF_PLUGIN_ABI_VERSION {
                Err(crate::error::PluginError::NativeError(format!(
                    "ABI version mismatch: plugin={}, host={}",
                    version, WF_PLUGIN_ABI_VERSION
                )))
            } else {
                Ok(())
            }
