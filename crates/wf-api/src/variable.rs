...
pub async fn get(
    ...
    ctx.storage
        ...
        .await?
        .ok_or_else(|| {
            not_found("variable", &variable_key(execution_id, scope, name))
        })
}
...
pub async fn list_by_scope(ctx: &ApiContext, scope: &str) -> ApiResult<Vec<VariableStorageMetadata>> {
    ctx.storage
        .variable
...
mod tests {
    ...
    async fn variable_crud_and_scoping() {
        ...
        let err = get(&ctx, "count", "other", Some("exec-1")).await.unwrap_err();
        assert!(matches!(err, ApiError::NotFound { .. }));

        assert!(delete(&ctx, "count", "local", Some("exec-1")).await.unwrap());
        assert!(!delete(&ctx, "count", "local", Some("exec-1")).await.unwrap());
    }
