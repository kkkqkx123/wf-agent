...
/// Global totals: `by_type`/`by_category` count unique metric names,
/// `total_metrics` counts buffered records.
...
fn detect_anomalies(registry: &MetricsRegistry) -> Vec<Anomaly> {
    ...
    if error_stats.total > thresholds.max_error_count {
    ...
    if workflow_stats.total > 0 && workflow_stats.success_rate < thresholds.min_success_rate {
        anomalies.push(Anomaly {
            ...
            severity: if workflow_stats.success_rate < thresholds.min_success_rate - 0.3 {
                Severity::High
            } else {
                Severity::Medium
            },
...
mod tests {
    ...
    use std::sync::Mutex;
    use wf_types::config::metrics::{AnomalyThresholdsConfig, MetricsConfig};
    ...
    #[tokio::test]
    async fn error_storm_threshold_is_configurable() {
        // 4 errors stays under the default threshold (100)...
        let default_registry = MetricsRegistry::new();
        for i in 0..4 {
            default_registry
                .error()
                .record_error("llm", "agent", Some(&format!("e{i}")));
        }
        let default_report = generate_report(&default_registry, &ReportOptions::default()).await;
        assert!(!default_report
            .anomalies
            .iter()
            .any(|a| { a.metric_name == crate::constants::error_metrics::OCCURRENCE_COUNT }));

        // ...but a configured `max_error_count` of 3 makes the same 4 errors
        // an anomaly (M6).
        let configured = MetricsRegistry::with_config(&MetricsConfig {
        ...
        let report = generate_report(&configured, &ReportOptions::default()).await;
        assert!(report
            .anomalies
            .iter()
            .any(|a| { a.metric_name == crate::constants::error_metrics::OCCURRENCE_COUNT }));
    ...
    #[tokio::test]
    async fn success_rate_threshold_is_configurable() {
        // 4/5 = 0.8 success rate is fine under the default threshold (0.8)...
        let default_registry = MetricsRegistry::new();
        for _ in 0..4 {
            default_registry.workflow().record_execution_start("wf-1");
            default_registry
                .workflow()
                .record_execution_complete("wf-1", None, true, 10.0, None);
        }
        default_registry.workflow().record_execution_start("wf-1");
        default_registry.workflow().record_execution_complete(
            "wf-1",
            None,
            false,
            10.0,
        ...
        assert!(!default_report
            .anomalies
            .iter()
            .any(|a| a.metric_name == "workflow.execution.success.rate"));

        // ...but a configured `min_success_rate` of 0.9 flags it as a
        // Medium-severity degradation (M6).
        let configured = MetricsRegistry::with_config(&MetricsConfig {
            anomaly_thresholds: Some(AnomalyThresholdsConfig {
        ...
        for _ in 0..4 {
            configured.workflow().record_execution_start("wf-1");
            configured
                .workflow()
                .record_execution_complete("wf-1", None, true, 10.0, None);
        }
        ...
        configured
            .workflow()
            .record_execution_complete("wf-1", None, false, 10.0, Some("timeout"));
        let report = generate_report(&configured, &ReportOptions::default()).await;
        let anomaly = report
            .anomalies
            .iter()
