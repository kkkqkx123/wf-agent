...
pub async fn get_execution_variables(
    ...
    let mut map = crate::variable::export(ctx, execution_id).await?;
...
pub async fn set_variable(
    ...
    crate::variable::set(ctx, name, "default", Some(execution_id), value).await
}
...
pub async fn delete_variable(ctx: &ApiContext, execution_id: &str, name: &str) -> ApiResult<bool> {
    crate::variable::delete(ctx, name, "default", Some(execution_id)).await
}
...
mod tests {
    ...
    async fn variables_upsert_query_and_export() {
        let ctx = make_ctx();
        set_variable(&ctx, "exec-v", "region", serde_json::json!("us-east")).await.unwrap();
        set_variable(&ctx, "exec-v", "replicas", serde_json::json!(3)).await.unwrap();
        ...
        assert_eq!(
            get_execution_variable(&ctx, "exec-v", "region").await.unwrap(),
            serde_json::json!("us-east")
        );
        assert!(has_execution_variable(&ctx, "exec-v", "region").await.unwrap());
        assert!(!has_execution_variable(&ctx, "exec-v", "missing").await.unwrap());
