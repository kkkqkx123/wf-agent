...
pub mod layertwine_impl {
    ...
    impl RemoteExecutor for LayertwineExecutor {
        ...
        async fn call(&self, method: &str, request: &Value) -> ToolResult<Value> {
            ...
            let result = {
                ...
                dispatch(client, method, request).await
            };
    ...
    impl LayertwineProcessManager {
        ...
        /// Blocking probe of the gRPC port.
        fn port_ready(&self) -> bool {
            TcpStream::connect_timeout(&self.parse_addr(), Duration::from_millis(500)).is_ok()
        }
