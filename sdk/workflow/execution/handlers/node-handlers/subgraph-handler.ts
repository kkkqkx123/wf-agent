  ...
  try {
    ...
    try {
      ...
      if (nodeCollector && typeof nodeCollector.recordSubgraphExecution === "function") {
        ...
        nodeCollector.recordSubgraphExecution(node.id, workflowExecutionEntity.getWorkflowId(), {
    ...
    try {
      ...
      // Don't fail the execution if metrics recording fails
      logger.warn("Failed to record subgraph failure metrics", {
        error: getErrorOrNew(metricsError),
      });
    }
