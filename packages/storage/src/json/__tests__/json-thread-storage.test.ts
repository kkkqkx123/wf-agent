...
describe("JsonThreadStorage", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "json-thread-test-"));
    storage = new JsonThreadStorage({ baseDir: tempDir });
  ...
  describe("updateThreadStatus", () => {
    ...
    it("should throw for non-existent thread", async () => {
      await expect(storage.updateThreadStatus("non-existent", "COMPLETED")).rejects.toThrow();
    });
  });
