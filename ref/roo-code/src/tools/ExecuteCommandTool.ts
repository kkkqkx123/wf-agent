...
import { CommandExecutionStatus, DEFAULT_TERMINAL_OUTPUT_PREVIEW_SIZE, PersistedCommandOutput } from "@coder/types"
...
import { ExitCodeDetails, RooTerminal, RooTerminalCallbacks, RooTerminalProcess } from "../../integrations/terminal/types"
...
export async function executeCommandInTerminal(
	...
	try {
		terminal = await TerminalRegistry.getOrCreateTerminal(targetWorkingDir, task.taskId, terminalProvider)
	} catch (error) {
		// If creating terminal with cd target fails, fall back to original working directory
		// This can happen when the cd target path doesn't exist
