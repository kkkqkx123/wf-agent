...
use crate::types::error::{ErrorCause, ErrorSeverity, ErrorType, RecoveryAction};
...
pub struct ErrorRecord {
    ...
    pub parent_error_id: Option<Id>,
    pub error_chain: Vec<Id>,
    pub root_cause_id: Id,
    pub caused_by: Option<ErrorCause>,
    pub is_recoverable: bool,
    pub recovery_action: Option<RecoveryAction>,
}
...
impl ErrorChainManager {
    ...
    pub fn record(&self, execution_id: Id, error: String, node_id: Option<String>) -> Id {
    ...
    pub fn get_chain(&self, execution_id: &Id) -> Vec<ErrorRecord> {
        ...
        let mut current_id: Option<&Id> = Some(&last.id);
    ...
    pub fn get_error_chain_ids(&self, execution_id: &Id) -> Vec<Id> {
        let all = self.get_records(execution_id);
        match all.last() {
    ...
    pub fn analyze_error_pattern(&self, execution_id: &Id) -> ErrorPattern {
    ...
    pub fn get_recommended_recovery_action(&self, execution_id: &Id) -> Option<RecoveryAction> {
...
mod tests {
    ...
    fn test_analyze_empty_pattern() {
        let manager = ErrorChainManager::new();
        let pattern = manager.analyze_error_pattern(&"nonexistent".to_string());
