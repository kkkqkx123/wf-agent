...
pub struct ScriptFlowEngine<'a> {
    scripts: &'a HashMap<String, Box<dyn Fn(&str) -> ExecutionSharedResult<String> + Send + Sync>>,
}
...
impl<'a> ScriptFlowEngine<'a> {
    ...
    where
        F: Fn(&str, &str) -> Fut,
        Fut: std::future::Future<Output = ExecutionSharedResult<String>>,
    {
...
mod tests {
    ...
    fn make_branch(key: &str, depends_on: Option<Vec<String>>, modules: Vec<&str>) -> FlowBranch {
        FlowBranch {
            ...
            modules: modules
                .into_iter()
                .map(|m| super::super::types::ModuleRef {
                    key: m.to_string(),
                    args: None,
                })
                .collect(),
