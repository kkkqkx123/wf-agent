...
type SkillsManagerLike = Pick<SkillsManager, "getEnabledSkillsForMode" | "isSkillsEnabled">
 ...
 * Only includes skills relevant to the current mode and enabled state.
 * Format matches the modes section style.
 *
 * @param skillsManager - The SkillsManager instance
 * @param currentMode - The current mode slug (e.g., 'code', 'architect')
 * @param skillsEnabled - Whether skills functionality is globally enabled (default: true)
 * @param disabledSkills - Optional array of disabled skill names
 */
export async function getSkillsSection(
	...
	// Check if skills functionality is globally enabled
	if (skillsEnabled === false || !skillsManager || !currentMode) {
		return ""
	}

	// Get skills filtered by current mode and enabled state
