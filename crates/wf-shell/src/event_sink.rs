...
//! **unbounded by design** (producers run on reader/PTY threads and must never
//! block; a bounded `send().await` is impossible there and `try_send` would
//! drop `Flush` barriers). Instead the queue is rate-limited by count per
//! event kind, so a wedged sink cannot grow memory without bound:
//!
//! - **Output** lines are capped at [`MAX_QUEUED_OUTPUT_EVENTS`]: once the
//!   backlog reaches the cap further lines are dropped and counted. A drop
//!   sentinel is synthesized on the next delivered output so a push consumer
//!   can fall back to the pull-side [`crate::engine::OutputBuffer`].
//! - **Lifecycle** events (`SessionCreated` / `CommandStarted` /
//!   `CommandCompleted` / `SessionTerminated`) are capped at
//!   [`MAX_QUEUED_LIFECYCLE_EVENTS`] (drops are logged).
//! - **`Flush`** barriers are never rate-limited: they are the reliable
//!   drain contract.
//!
//! The caps are per-kind, so lifecycle traffic never eats the output budget.
//! Total memory bound: 4096 output + 256 lifecycle events. The dispatcher is
//! created once per [`crate::engine::BackgroundShellStore`] when a sink is
//! configured, keeping the extra thread count bounded.

use std::sync::atomic::{AtomicUsize, Ordering};
use std::sync::Arc;
use std::time::Duration;

/// Cap on queued `Output` events per dispatcher. This is a true per-kind cap:
/// lifecycle/`Flush` events do not count against it (they have their own
/// budget). The bound is approximate by event count, not bytes.
const MAX_QUEUED_OUTPUT_EVENTS: usize = 4096;

/// Cap on queued lifecycle events (`SessionCreated` / `CommandStarted` /
/// `CommandCompleted` / `SessionTerminated`) per dispatcher. Overflow is
/// dropped with a warning; `Flush` barriers are never rate-limited.
const MAX_QUEUED_LIFECYCLE_EVENTS: usize = 256;
...
pub(crate) struct EventDispatcher {
    ...
    /// traffic never eats the output budget.
    queued_outputs: Arc<AtomicUsize>,
    /// Number of lifecycle events currently queued. Drives the lifecycle cap.
    queued_lifecycle: Arc<AtomicUsize>,
...
/// Event kind for queue accounting: each kind has its own cap, `Flush` is
/// never rate-limited.
...
fn event_kind(event: &ShellEvent) -> EventKind {
    match event {
        ShellEvent::Output { .. } => EventKind::Output,
        ShellEvent::SessionCreated { .. }
        | ShellEvent::CommandStarted { .. }
        | ShellEvent::CommandCompleted { .. }
        | ShellEvent::SessionTerminated { .. } => EventKind::Lifecycle,
        ShellEvent::Flush { .. } => EventKind::Flush,
    }
}
...
impl EventDispatcher {
    ...
    fn spawn(sink: Arc<dyn ShellEventSink>, max_queued_output_events: usize) -> Arc<Self> {
        ...
        let queued_outputs = Arc::new(AtomicUsize::new(0));
        let queued_lifecycle = Arc::new(AtomicUsize::new(0));
        let dropped = Arc::new(AtomicUsize::new(0));
        let thread_queued_outputs = Arc::clone(&queued_outputs);
        let thread_queued_lifecycle = Arc::clone(&queued_lifecycle);
            ...
            .spawn(move || {
                while let Some(event) = rx.blocking_recv() {
                    match event_kind(&event) {
                        EventKind::Output => {
                            thread_queued_outputs.fetch_sub(1, Ordering::SeqCst);
                        }
                        EventKind::Lifecycle => {
                            thread_queued_lifecycle.fetch_sub(1, Ordering::SeqCst);
                        }
                        EventKind::Flush => {}
                    }
    ...
    /// Each kind has its own cap: `Output` events are dropped (and counted)
    /// once the output backlog reaches the cap, lifecycle events once they
    /// reach [`MAX_QUEUED_LIFECYCLE_EVENTS`] (drops are logged). `Flush`
    /// barriers are never rate-limited.
    pub(crate) fn send(&self, event: ShellEvent) {
        match event_kind(&event) {
            EventKind::Output => {
                if self.queued_outputs.load(Ordering::SeqCst) >= self.max_queued_output_events {
                    self.dropped.fetch_add(1, Ordering::SeqCst);
                    return;
                }
            }
            EventKind::Lifecycle => {
                if self.queued_lifecycle.load(Ordering::SeqCst) >= MAX_QUEUED_LIFECYCLE_EVENTS {
                    tracing::warn!("shell event dispatcher dropped a lifecycle event (queue full)");
                    return;
                }
            }
            EventKind::Flush => {}
    ...
    /// Queue an event and bump the per-kind in-queue counter. On a closed
    /// queue the counters are rolled back so accounting stays exact.
    fn enqueue(&self, event: ShellEvent) -> bool {
        let kind = event_kind(&event);
        match kind {
            EventKind::Output => {
                self.queued_outputs.fetch_add(1, Ordering::SeqCst);
            }
            EventKind::Lifecycle => {
                self.queued_lifecycle.fetch_add(1, Ordering::SeqCst);
            }
            EventKind::Flush => {}
        }
        ...
        match kind {
            EventKind::Output => {
                self.queued_outputs.fetch_sub(1, Ordering::SeqCst);
            }
            EventKind::Lifecycle => {
                self.queued_lifecycle.fetch_sub(1, Ordering::SeqCst);
            }
            EventKind::Flush => {}
        }
        false
...
mod tests {
    ...
    /// Lifecycle events must not eat the output budget: the output cap is
    /// per-kind, so an output that arrives behind a flood of lifecycle events
    /// is still accepted.
    #[test]
    fn test_lifecycle_events_do_not_eat_the_output_budget() {
        let (sink, entered_rx, release_tx) = GatedSink::new();
        let dispatcher = EventDispatcher::with_output_capacity(sink.clone(), 2);
        ...
        // "2" fills the output slot; then flood lifecycle events and send a
        // third output: it must still be accepted (lifecycle has its own cap).
        dispatcher.send(ShellEvent::Output {
            session_id: "s1".into(),
            task_id: None,
            line: "2".into(),
        ...
        let events = sink.recorded();
        assert!(events.iter().any(|e| e == "s1|1"), "{:?}", events);
        assert!(events.iter().any(|e| e == "s1|2"), "{:?}", events);
        assert!(
            events.iter().any(|e| e == "s1|3"),
            "output budget must be output-only: {:?}",
            events
        );
        assert_eq!(
            events.iter().filter(|e| e.ends_with("|created")).count(),
            100,
            "all lifecycle events delivered: {:?}",
            events
        );
    }
