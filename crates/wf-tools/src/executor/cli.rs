...
pub struct ExecutorConfig {
    ...
    /// Optional sandbox policy applied to the spawned binary through the
    /// shared `wf-sandbox` execution gateway (seccomp + rlimits + env
    /// clearing). `None` runs the binary unsandboxed.
...
pub trait CliExecutor: Send + Sync {
    ...
    async fn execute(&self, options: &CliExecutionOptions) -> ToolResult<CliExecutionResult> {
        ...
        // External binaries run through the shared sandbox gateway when a
        // policy is configured: kernel-level enforcement without duplicating
        // the seccomp/rlimit logic.
        if let Some(policy) = self.config().sandbox_policy.as_ref() {
            wf_sandbox::cmd::apply_tokio(
                &mut cmd,
                policy,
                &wf_sandbox::ApplyOptions {
                    clear_env: true,
                    path_policy: None,
                    cwd: options.cwd.clone().map(std::path::PathBuf::from),
                },
            )
            .map_err(|e| ToolError::Internal(format!("Failed to sandbox CLI tool: {e}")))?;
        }
