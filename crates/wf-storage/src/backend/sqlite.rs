...
fn to_sqlite_url(path: &str) -> String {
    if path.starts_with("sqlite:") { path.to_string() }
    else if path == ":memory:" || path == "file::memory:" { "sqlite::memory:".to_string() }
    else if path.starts_with('/') { format!("sqlite://{}", path) }
    else { format!("sqlite:{}", path) }
}
...
impl SqliteStorage {
    pub async fn new(path: &str, table_name: &str) -> Result<Self, StorageError> {
        ...
        let create_sql = format!(
            "CREATE TABLE IF NOT EXISTS {} (id TEXT PRIMARY KEY, data TEXT NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)",
            table_name
        );
...
impl BaseStorageAdapter<serde_json::Value, ListOptions> for SqliteStorage {
    ...
    async fn save(&self, entity: &serde_json::Value) -> Result<(), StorageError> {
        ...
        let sql = format!("INSERT OR REPLACE INTO {} (id, data, created_at, updated_at) VALUES (?1, ?2, ?3, ?4)", self.table_name);
    ...
    async fn load(&self, id: &str) -> Result<Option<serde_json::Value>, StorageError> {
        ...
        let result: Option<(String,)> = sqlx::query_as(&sql).bind(id)
            .fetch_optional(&self.pool).await.map_err(|e| StorageError::ReadFailed(e.to_string()))?;
        result.map(|r| serde_json::from_str(&r.0).map_err(Into::into)).transpose()
    }
    ...
    async fn list(&self, _options: Option<ListOptions>) -> Result<Vec<serde_json::Value>, StorageError> {
        ...
        rows.into_iter().map(|r| serde_json::from_str(&r.0).map_err(Into::into)).collect()
    }
...
impl SqliteWorkflowStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        Ok(Self { inner: SqliteStorage::new(path, "workflow_templates").await? })
    }
}

impl BaseStorageAdapter<wf_types::WorkflowTemplate, WorkflowListOptions> for SqliteWorkflowStorage {
...
impl WorkflowStorageAdapter for SqliteWorkflowStorage {
...
pub struct SqliteWorkflowExecutionStorage {
    inner: SqliteStorage,
}

impl SqliteWorkflowExecutionStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        Ok(Self { inner: SqliteStorage::new(path, "workflow_executions").await? })
    }
}

impl BaseStorageAdapter<wf_types::WorkflowExecution, WorkflowExecutionListOptions> for SqliteWorkflowExecutionStorage {
    async fn initialize(&self) -> Result<(), StorageError> { self.inner.initialize().await }
    async fn close(&self) -> Result<(), StorageError> { self.inner.close().await }
...
impl WorkflowExecutionStorageAdapter for SqliteWorkflowExecutionStorage {
...
impl SqliteCheckpointStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        Ok(Self { inner: SqliteStorage::new(path, "checkpoints").await? })
    }
}

impl BaseStorageAdapter<wf_types::Checkpoint, CheckpointListOptions> for SqliteCheckpointStorage {
    async fn initialize(&self) -> Result<(), StorageError> { self.inner.initialize().await }
    async fn close(&self) -> Result<(), StorageError> { self.inner.close().await }
...
impl CheckpointStorageAdapter for SqliteCheckpointStorage {
...
impl SqliteTaskStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        Ok(Self { inner: SqliteStorage::new(path, "tasks").await? })
    }
}

impl BaseStorageAdapter<wf_types::TaskStorageMetadata, TaskListOptions> for SqliteTaskStorage {
    async fn initialize(&self) -> Result<(), StorageError> { self.inner.initialize().await }
    async fn close(&self) -> Result<(), StorageError> { self.inner.close().await }
...
impl TaskStorageAdapter for SqliteTaskStorage {
...
pub struct SqliteAgentLoopStorage {
    inner: SqliteStorage,
}

impl SqliteAgentLoopStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        Ok(Self { inner: SqliteStorage::new(path, "agent_loops").await? })
    }
}

impl BaseStorageAdapter<wf_types::AgentLoopStorageMetadata, AgentLoopListOptions> for SqliteAgentLoopStorage {
    async fn initialize(&self) -> Result<(), StorageError> { self.inner.initialize().await }
    async fn close(&self) -> Result<(), StorageError> { self.inner.close().await }
...
impl AgentLoopStorageAdapter for SqliteAgentLoopStorage {
...
impl SqliteMetricsStorage {
    pub async fn new(path: &str) -> Result<Self, StorageError> {
        ...
        let pool = SqlitePoolOptions::new().max_connections(1)
            .connect(&url).await.map_err(|e| StorageError::ConnectionFailed(e.to_string()))?;

        sqlx::query("CREATE TABLE IF NOT EXISTS metrics (name TEXT NOT NULL, value REAL NOT NULL, timestamp BIGINT NOT NULL, tags TEXT)")
...
impl MetricsStorageAdapter for SqliteMetricsStorage {
    async fn save_batch(&self, points: &[MetricsDataPoint]) -> Result<(), StorageError> {
        ...
        for point in points {
            let tags_json = point.tags.as_ref().map(|t| serde_json::to_string(t).unwrap_or_default());
    ...
    async fn query(&self, name: &str, start_time: i64, end_time: i64) -> Result<Vec<MetricsDataPoint>, StorageError> {
        ...
        rows.into_iter().map(|(n, v, ts, tag_json)| {
            let tags = tag_json.and_then(|j| serde_json::from_str::<HashMap<String, String>>(&j).ok());
...
// ============================================================================
// Simple CRUD Adapters (via EntityStore<SqliteStorage, T>)
// ============================================================================

pub type SqliteAgentProfileStorage = EntityStore<SqliteStorage, wf_types::AgentProfileStorageMetadata>;
pub type SqliteScriptStorage = EntityStore<SqliteStorage, wf_types::ScriptStorageMetadata>;
pub type SqliteToolStorage = EntityStore<SqliteStorage, wf_types::ToolStorageMetadata>;
pub type SqliteHookTemplateStorage = EntityStore<SqliteStorage, wf_types::HookTemplateStorageMetadata>;
pub type SqliteNodeTemplateStorage = EntityStore<SqliteStorage, wf_types::NodeTemplateStorageMetadata>;
pub type SqliteTriggerStorage = EntityStore<SqliteStorage, wf_types::TriggerStorageMetadata>;
pub type SqliteFileCheckpointStorage = EntityStore<SqliteStorage, wf_types::FileCheckpointStorageMetadata>;

impl FileCheckpointStorageAdapter for SqliteFileCheckpointStorage {
