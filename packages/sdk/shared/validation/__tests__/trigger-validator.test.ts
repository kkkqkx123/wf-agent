...
describe("validateTriggers", () => {
  ...
  it("should detect duplicate trigger IDs", () => {
    ...
    expect(result.isErr()).toBe(true);
    if (result.isErr()) {
      const hasDuplicateError = result.error.some(e => e.message.includes("must be unique"));
  ...
  it("should reject null/undefined triggers in the array", () => {
    const triggers = [createValidTrigger({ id: "trigger-1", name: "Trigger 1" }), null, undefined];
    const result = validateTriggers(triggers as any);
