...
/// Result of [`ExecutionBuilder::execute_with_result`]: the execution output
/// projected together with its terminal status and duration.
...
pub struct ExecutionResult {
    ...
    /// Wall-clock duration computed from the persisted record's start/end
    /// timestamps; `None` while the execution is still running.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub duration_ms: Option<i64>,
}
...
impl ExecutionBuilder {
    ...
    /// Execute the workflow and project the output together with the terminal
    /// status and wall-clock duration ([`ExecutionResult`]).
    ///
    /// Delegates to [`ExecutionBuilder::execute`] and reads the persisted
    /// execution record for the status and the start/end timestamps, so the
    /// engine itself is unchanged.
    pub async fn execute_with_result(self, ctx: &Arc<ApiContext>) -> ApiResult<ExecutionResult> {
        let output = self.execute(ctx).await?;
        let record = crate::workflow::get_execution(ctx, &output.execution_id.to_string()).await?;
        let duration_ms = match (record.started_at, record.completed_at) {
    ...
    pub async fn execute_stream(
        ...
        let execution_id_filter = execution_id.to_string();
        let callback_key = format!("{execution_id_filter}:callbacks");
        ...
        // The forwarder loops until the bus is destroyed; track it in the
        // same registry so teardown (`ctx.shutdown`) aborts it too.
        ctx.execution_tasks
            .register(callback_key, handle.abort_handle());
...
mod tests {
    ...
    #[tokio::test]
    async fn execute_with_result_projects_status_and_duration() {
        let ctx = make_ctx();
        ctx.storage
            .workflow
            .save(&make_workflow("wf-builder-exec-result"))
            .await
            .unwrap();

        let result = ExecutionBuilder::new("wf-builder-exec-result")
            .with_input(serde_json::json!({"greeting": "hi"}))
            .execute_with_result(&ctx)
            .await
            .expect("workflow completes");
        assert_eq!(
            result.status,
            wf_types::ExecutionStatus::Completed,
            "execution must finish as completed"
        );
        assert_eq!(result.output, Some(serde_json::json!({"greeting": "hi"})));
        assert!(
            result.duration_ms.is_some(),
            "duration must be projected from the persisted record"
        );
    }
