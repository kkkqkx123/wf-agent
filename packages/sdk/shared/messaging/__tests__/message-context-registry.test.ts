...
describe("InMemoryMessageContextRegistry", () => {
  ...
  function createContext(
    id: string,
    overrides: Partial<NamedMessageContext> = {},
  ): NamedMessageContext {
    return {
      id,
      messages: [],
  ...
  describe("getAll", () => {
    ...
    it("should return all contexts", () => {
      registry.register(
        createContext("a", { metadata: { key: "val1" } as Record<string, unknown> }),
      );
      registry.register(
        createContext("b", { metadata: { key: "val2" } as Record<string, unknown> }),
      );
