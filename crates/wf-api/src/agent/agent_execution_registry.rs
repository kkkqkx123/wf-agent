...
use crate::agent::agent_loop_registry::{AgentExecutionStatistics, AgentLoopStatistics};
use crate::infra::context::ApiContext;
use crate::infra::error::ApiResult;

/// Agent execution query filter.
...
pub async fn execution_statistics(ctx: &ApiContext) -> ApiResult<AgentExecutionStatistics> {
    let summaries = crate::agent::agent_loop_registry::summaries(ctx, None).await?;
    Ok(crate::agent::agent_loop_registry::aggregate_execution_statistics(&summaries))
}
...
pub async fn has(ctx: &ApiContext, agent_loop_id: &str) -> ApiResult<bool> {
    Ok(ctx
        .agent_loops
        .has(&wf_types::Id::from(agent_loop_id.to_string()))
        || ctx
            .storage
            .agent_execution
            .load(agent_loop_id)
...
/// Total number of distinct executions (live registries plus persisted
...
pub async fn status_statistics(ctx: &ApiContext) -> ApiResult<AgentLoopStatistics> {
    crate::agent::agent_loop_registry::statistics(ctx).await
}
    ...
    async fn register_loop(ctx: &ApiContext, id: &str, status: ExecutionStatus) {
        let entity = Arc::new(
            AgentLoopEntity::new(Id::from(id.to_string()))
                .with_definition_id(Id::from("agent-def-1".to_string())),
        );
        {
