...
| [08-wf-server-HTTP层.md](./08-wf-server-HTTP层.md) | wf-server：三子目录路由结构、421 端点清单、信封/SSE/WS/中间件/配置 |
...
- **wf-server** 是纯 HTTP 传输层（axum），421 条路由 / 38 个域文件，`api/` 分 `workflow`/`agent`/`resource` 三子目录；全部逻辑委托给 wf-api，自身只做信封封装、SSE/WS 帧、中间件与分层配置加载。文件级 checkpoint 与 webhook 入站网关为新增面。
