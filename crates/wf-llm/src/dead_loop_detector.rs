...
impl DeadLoopDetector {
    ...
    pub fn detect(&mut self, reasoning_message: &str) -> DeadLoopDetectionResult {
        let char_count = reasoning_message.len() as u32;
    ...
    fn detect_at_checkpoint(
        &self,
        text: &str,
        checkpoint: u32,
    ) -> Option<DeadLoopDetectionResult> {
        let previous_checkpoint = self.get_previous_checkpoint(checkpoint);
        let start_index = previous_checkpoint.unwrap_or(0) as usize;
        ...
        if self.config.checkpoints.len() >= 2
            && checkpoint >= self.config.checkpoints[1]
        {
    ...
    fn detect_short_sequence(&self, text: &str) -> Option<DeadLoopDetectionResult> {
        let window_size = std::cmp::min(self.config.short_sequence_window as usize, text.len());
        let recent_text = if text.len() >= window_size {
            &text[text.len() - window_size..]
        } else {
            text
        };
        ...
        for unit_len in min_unit..=recent_text.len() / min_count {
            for start in 0..=recent_text.len() - unit_len * min_count {
                let unit = &recent_text[start..start + unit_len];
                let mut consecutive = 1;
                for i in 1..min_count {
                    let next_start = start + i * unit_len;
                    if next_start + unit_len <= recent_text.len()
                        && &recent_text[next_start..next_start + unit_len] == unit
                    {
                        consecutive += 1;
                    } else {
                        break;
    ...
    fn detect_paragraph_repeat(&self, text: &str) -> Option<DeadLoopDetectionResult> {
        ...
        if let Some(period) = self.detect_period(&blocks) {
            return Some(DeadLoopDetectionResult {
                ...
                details: Some(format!(
                    "Detected paragraph repeat with period {}",
                    period
                )),
            });
        }
    ...
    fn detect_period<T: PartialEq>(&self, elements: &[T]) -> Option<usize> {
        let max_period = std::cmp::min(
            self.config.max_period_length as usize,
            elements.len() / 2,
        );
