...
impl MessageContextRegistry {
    ...
    pub fn update(&self, name: &str, messages: Vec<Message>) -> Result<(), crate::error::ExecutionSharedError> {
        ...
        match contexts.get_mut(name) {
            ...
            None => Err(crate::error::ExecutionSharedError::StateError(format!(
    ...
    pub fn set_metadata(&self, name: &str, metadata: HashMap<String, Value>) -> Result<(), crate::error::ExecutionSharedError> {
        let mut contexts = self.contexts.lock().unwrap();
        match contexts.get_mut(name) {
            Some(ctx) => {
