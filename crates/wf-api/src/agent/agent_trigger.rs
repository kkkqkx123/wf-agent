...
use crate::entity::trigger::{get_trigger, list_triggers, list_triggers_by_event};
...
pub async fn register(ctx: &ApiContext, trigger: &TriggerStorageMetadata) -> ApiResult<()> {
    if ctx.storage.trigger.load(&trigger.id).await?.is_some() {
...
mod tests {
    ...
    async fn list_get_enable_disable_and_register() {
        ...
        register(&ctx, &make_trigger("tr-2", "schedule"))
            .await
            .unwrap();
        ...
        let err = register(&ctx, &make_trigger("tr-1", "push"))
            .await
            .unwrap_err();
