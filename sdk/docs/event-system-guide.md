...
const slowListeners = listeners.filter(l => 
  l.metrics?.slowExecutionCount > 10
);
```
...
sdk.events.on('NODE_COMPLETED', async (event) => {
...
const listeners = sdk.events.getAllListenerInfo();
const problematicListeners = listeners.filter(l => 
  l.metrics && 
  l.metrics.failureCount > 5 &&
  l.metrics.totalExecutions > 10
);
...
function trackExecutionProgress(executionId: string) {
  ...
  const unsubscribe = sdk.events.on('NODE_COMPLETED', (event) => {
...
function aggregateErrors(executionId: string) {
  ...
  sdk.events.on('NODE_FAILED', (event) => {
    if (event.executionId === executionId) {
      errors.push({
        nodeId: event.nodeId,
