 ...
 *
 * Design Notes:
 * - Contains preprocessing and analysis data for workflow graphs
 * - Separated from core graph structure for clean separation of concerns
 * - Mutable during preprocessing phase, immutable after completion
 *
 * Core Responsibilities:
 * - Stores ID mappings and preprocessing results
 * - Contains graph analysis data (cycles, reachability, etc.)
 * - Manages workflow metadata and validation results
 * - Tracks subgraph relationships and merge logs
 *
 * Usage:
 * - Used during workflow preprocessing phase
 * - Stored in WorkflowGraphRegistry after preprocessing
 * - Provides metadata for graph optimization and debugging
 */
...
/**
 * Workflow Graph Metadata
 * Contains preprocessing and analysis data separated from core graph structure
 */
export class WorkflowGraphMetadata {
  // ========== ID Mapping Related ==========
   ...
   * @returns True if preprocessing has been completed
   */
  public isPreprocessed(): boolean {
    return this.processedAt > 0 && this.validationResult.validatedAt > 0;
  }
   ...
   */
  public markPreprocessed(timestamp: Timestamp): void {
    this.processedAt = timestamp;
    this.validationResult.validatedAt = timestamp;
  }
}