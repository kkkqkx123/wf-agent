/**
 * JSON File Hook Template Storage Implementation
  ...
  override async list(options?: HookTemplateListOptions): Promise<string[]> {
    ...
    if (options) {
      ids = ids.filter(id => {
        ...
        if (options.hookType) {
          const types = Array.isArray(options.hookType) ? options.hookType : [options.hookType];
          if (!types.includes(metadata.hookType)) {
            return false;
          }
        }
    ...
    ids.sort((a, b) => {
      ...
      switch (sortBy) {
        case "name":
          ...
          break;
        case "hookType":
          valueA = metaA?.hookType ?? "";
          valueB = metaB?.hookType ?? "";
          break;
        case "updatedAt":
