...
import { RooIgnoreController, LOCK_TEXT_SYMBOL, IgnoreMode } from "../../ignore/RooIgnoreController"
...
describe("RooIgnore Response Formatting", () => {
	...
	describe("formatResponse.formatFilesList with RooIgnoreController", () => {
		...
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller with gitignore enabled
			const controller = new RooIgnoreController(TEST_CWD, IgnoreMode.Both)
			...
			// Should contain only allowed files
		...
		/**
		 * Tests formatFilesList always hides ignored files
		 */
		it("should hide ignored files", async () => {
			// Create controller with gitignore enabled
			const controller = new RooIgnoreController(TEST_CWD, IgnoreMode.Both)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		...
		it("should return undefined when no .rooignore exists", async () => {
			...
			// Create controller without .rooignore (gitignore enabled)
			const controller = new RooIgnoreController(TEST_CWD, IgnoreMode.Both)
			await controller.initialize()
