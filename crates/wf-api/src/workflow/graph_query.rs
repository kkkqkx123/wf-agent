//! Workflow graph structure query API (TS `WorkflowGraphQueryAPI`
//! counterpart).
//!
...
// ── execution graph queries (TS `WorkflowExecutionGraphQueryAPI`) ──────
...
/// Path statistics of an execution (TS `getPathStatistics`): the resolved
/// execution paths and the node-type distribution.
...
mod tests {
    ...
    use crate::workflow::workflow::save_workflow;
