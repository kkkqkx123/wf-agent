...
describe('llmHandler', () => {
  ...
  it('should write response to outputContext when configured', async () => {
    mockRegistry.get.mockImplementation((id: string) => {
      if (id === 'current') return { id: 'current', messages: [{ role: 'user', content: 'hi' }], createdAt: Date.now(), updatedAt: Date.now() };
      if (id === 'output-ctx') return { id: 'output-ctx', messages: [], createdAt: Date.now(), updatedAt: Date.now() };
      return undefined;
    });
    ...
    expect(mockRegistry.update).toHaveBeenCalledWith('output-ctx', expect.arrayContaining([
