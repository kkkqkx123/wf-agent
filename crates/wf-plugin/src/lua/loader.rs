...
async fn load_lua_plugin_at(manifest: &PluginManifest, base_path: &Path) -> PluginResult<Arc<dyn Plugin>> {
...
fn apply_sandbox(lua: &mlua::Lua) -> PluginResult<()> {
    ...
    let safe_require = lua.create_function(|_, module_name: String| -> mlua::Result<mlua::Value> {
        Err(mlua::Error::RuntimeError(format!("module '{}' not allowed in plugin sandbox", module_name)))
    }).map_err(|e| PluginError::LuaError(e.to_string()))?;
