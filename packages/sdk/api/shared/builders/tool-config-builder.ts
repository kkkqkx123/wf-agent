/**
 * ToolConfigBuilder - Fluent builder for tool configuration
 *
 * Shared builder for AgentToolConfig, used by both Agent and Workflow domains.
 * Moved from agent/builders/agent-tool-config-builder.ts to support both domains.
 * Workflow agent-loop nodes use the same AgentToolConfig type.
 */

import type { AgentToolConfig } from "@wf-agent/types";

/**
 * ToolConfigBuilder - Build tool configuration with fluent API
 */
export class ToolConfigBuilder {
  ...
  /**
   * Create a new ToolConfigBuilder instance
   * @returns ToolConfigBuilder instance
   */
  static create(): ToolConfigBuilder {
   ...
   */
  clone(): ToolConfigBuilder {
    const cloned = new ToolConfigBuilder();
