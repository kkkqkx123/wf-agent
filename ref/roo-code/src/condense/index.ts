...
export async function summarizeConversation(options: SummarizeConversationOptions): Promise<SummarizeResponse> {
	...
	const requestMessages = messagesWithTextToolBlocks
		.filter((msg) => msg.role !== "system")
		.map(({ role, content }) => ({ role, content })) as Anthropic.Messages.MessageParam[]
...
export function getEffectiveApiHistory(messages: ApiMessage[]): ApiMessage[] {
	...
	const lastSummary = findLast(messagesWithoutSystemPrompt, (msg) => msg.isSummary === true)

	if (lastSummary) {
		// Fresh start model: return only messages from the summary onwards
		const summaryIndex = messagesWithoutSystemPrompt.indexOf(lastSummary)
		let messagesFromSummary = messagesWithoutSystemPrompt.slice(summaryIndex)
	...
	for (const msg of messagesWithoutSystemPrompt) {
	...
	return messagesWithoutSystemPrompt.filter((msg) => {
