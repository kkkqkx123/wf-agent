...
pub async fn analyze(ctx: &ApiContext, agent_loop_id: &str) -> ApiResult<AgentIterationAnalysis> {
    let (iterations, tool_calls, llm_only_iterations): (Vec<u32>, Vec<(String, i64, bool)>, u32) =
...
mod tests {
    ...
    async fn aggregates_iteration_metrics() {
        ...
        let analysis = analyze(&ctx, "agent-iter-1").await.unwrap();
