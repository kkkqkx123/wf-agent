...
/// right after start, then stdin is closed. When `policy` is `Some` the
/// command runs through the shared `wf-sandbox` execution gateway.
pub async fn run_command(
    ...
    input: Option<&str>,
    policy: Option<&wf_types::script::sandbox::SandboxPolicy>,
...
mod tests {
    ...
    async fn test_run_command_echo() {
        let output = run_command("echo hello", None, DEFAULT_TIMEOUT_MS, None, None, None)
            .await
            .unwrap();
    ...
    async fn test_run_command_timeout() {
        let result = run_command("sleep 10", None, 200, None, None, None).await;
