...
/// Upper bound for enqueuing a request into the transport's bounded channel.
/// The transport writer normally drains this immediately; the timeout only
/// fires when the writer is wedged and the RPC could never be served.
...
impl McpClient {
    ...
    pub async fn connect(&self) -> ToolResult<Value> {
        ...
        *wf_common::lock::lock_ok(self.request_tx.lock()) = Some(handle.request_tx);
        ...
        if let Some(instructions) = init.get("instructions").and_then(|v| v.as_str()) {
            *wf_common::lock::lock_ok(self.instructions.lock()) = Some(instructions.to_string());
        }
    ...
    pub async fn disconnect(&self) -> ToolResult<()> {
        if let Some(handle) = wf_common::lock::lock_ok(self.dispatcher.lock()).take() {
            handle.abort();
        }
        self.reject_all_pending("Connection closed");
        *wf_common::lock::lock_ok(self.request_tx.lock()) = None;
        let mut transport = self.transport.lock().await;
    ...
    pub fn instructions(&self) -> Option<String> {
        wf_common::lock::lock_ok(self.instructions.lock()).clone()
    }
    ...
    fn spawn_dispatcher(&self, response_rx: mpsc::Receiver<ToolResult<JsonRpcResponse>>) {
        ...
        *wf_common::lock::lock_ok(self.dispatcher.lock()) = Some(dispatcher);
    }
    ...
    async fn call_method(&self, method: &str, params: Value, timeout_ms: u64) -> ToolResult<Value> {
        ...
        let Some(request_tx) = wf_common::lock::lock_ok(self.request_tx.lock()).clone() else {
            self.pending.remove(&id);
            return Err(ToolError::McpError("Transport not connected".into()));
        };

        // Bound the enqueue: a wedged transport writer (stuck on a full OS
        // pipe) would otherwise leave this `send` blocked forever regardless
        // of `timeout_ms`. The request-channel is bounded (64), so the timeout
        // only fires when the writer stops draining.
        let send_result = tokio::time::timeout(
            std::time::Duration::from_millis(SEND_TIMEOUT_MS),
        ...
        match send_result {
            ...
            _ => {
                ...
                return Err(ToolError::McpError(
                    "Request channel closed or send timed out".into(),
                ));
            }
        }
