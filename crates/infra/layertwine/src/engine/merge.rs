...
pub fn apply_deltas(content: &str, deltas: &[Delta]) -> Result<String> {
    ...
    // The trailing-newline state is that of the *last* delta that actually
    // changed content (line tokens are stored newline-stripped, so the flag
    // recorded at diff time is authoritative); fall back to the base content
    // when no such delta exists (empty / hand-built / legacy diffs).
    let has_trailing_newline = deltas
        .iter()
        .rev()
        .find(|d| d.diff.trailing_newline.is_some())
        .and_then(|d| d.diff.trailing_newline)
        .unwrap_or_else(|| content.ends_with('\n'));
