/**
 * WorkflowExecutionPool - Workflow Execution Pool Service (Global Singleton Service)
 *
 * Responsibilities:
 * - Manages the creation, allocation, and reclamation of WorkflowExecutor instances.
 * - Implements dynamic scaling.
 * - Maintains a queue of idle executors and a set of busy executors.
 * - Provides unified workflow execution pool resource management for the entire system.
 *
 * Design Principles:
...
/**
 * WorkflowExecutionPool - Workflow Execution Pool Service (Global Singleton)
 */
export class WorkflowExecutionPool {
  ...
  private waitingPromises: Array<{
    resolve: (executor: WorkflowExecutor) => void;
    reject: (error: Error) => void;
  }> = [];

  /**
   * WorkflowExecutor factory function
   */
  private executorFactory: () => WorkflowExecutor;
   ...
   */
  static getInstance(
    executorFactory: () => WorkflowExecutor,
    config?: ExecutionPoolConfig,
  ): WorkflowExecutionPool {
    if (!WorkflowExecutionPool.instance) {
      WorkflowExecutionPool.instance = new WorkflowExecutionPool(executorFactory, config);
    }
    return WorkflowExecutionPool.instance;
  }
  ...
  static resetInstance(): void {
    if (WorkflowExecutionPool.instance) {
      WorkflowExecutionPool.instance.shutdown();
      WorkflowExecutionPool.instance = null;
    }
  }
  ...
  async allocateExecutor(): Promise<WorkflowExecutor> {
    ...
    if (this.idleExecutors.length > 0) {
      ...
      return wrapper.executor as WorkflowExecutor;
    }
    ...
    return new Promise<WorkflowExecutor>((resolve, reject) => {
   ...
   */
  async releaseExecutor(executor: WorkflowExecutor): Promise<void> {
    if (this.isShutdown) {
      return;
    }
