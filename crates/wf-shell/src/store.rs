...
//! command so later commands can reuse the same session.
...
pub struct BackgroundShellStore {
    ...
    /// Optional sandbox policy for every spawned session command. The pipe
    /// backend enforces it via the shared `wf-sandbox` gateway; the PTY
    /// backend cannot attach `pre_exec` hooks and skips kernel enforcement.
    sandbox_policy: Option<wf_types::script::sandbox::SandboxPolicy>,
    max_timeout_ms: u64,
...
impl BackgroundShellStore {
    ...
    pub fn from_config(config: &crate::config::ShellToolConfig) -> Self {
        ...
        store.sandbox_policy = config.sandbox_policy.clone();
    ...
    ///
    /// A reused session adopts the requested `task_id`.
            ...
            if let Some(tid) = task_id {
                for entry in self.sessions.iter() {
                    ...
                    if session.status() == SessionStatus::Idle
                        && wf_common::lock::lock_ok(session.task_id.lock()).as_deref() == Some(tid)
                        && session
                            .cwd
                            .as_ref()
                            .map(|c| normalize_cwd_path(&c.to_string_lossy()))
    ...
    pub fn send_input(&self, session_id: &str, input: &str, enter: bool) -> ShellResult<()> {
        ...
        let current = wf_common::lock::lock_ok(session.current.lock())
            .clone()
            .ok_or_else(|| {
    ...
    /// reusable; with `terminate=true`
    /// running commands are killed and the sessions removed. Returns the
        ...
        for entry in self.sessions.iter() {
            ...
            if terminate {
                ...
                *wf_common::lock::lock_ok(session.task_id.lock()) = None;
                ...
                if !has_running {
                    *wf_common::lock::lock_ok(session.status.lock()) = SessionStatus::Idle;
                }
                *wf_common::lock::lock_ok(session.last_active_at.lock()) = wf_common::time::now();
            }
        }
        for id in to_remove {
    ...
    fn reuse_session(
        ...
        if let Some(tid) = task_id {
            *wf_common::lock::lock_ok(session.task_id.lock()) = Some(tid.to_string());
        }
        *wf_common::lock::lock_ok(session.last_active_at.lock()) = wf_common::time::now();
    ...
    fn create_session(
        ...
        if let (Some(max), Some(tid)) = (self.max_sessions_per_task, task_id) {
            let count = self
                ...
                .iter()
                .filter(|e| {
                    wf_common::lock::lock_ok(e.value().task_id.lock()).as_deref() == Some(tid)
                })
                .count();
            if count >= max {
                return Err(ShellError::ExecutionError(format!(
    ...
    fn spawn_command(
        ...
        *wf_common::lock::lock_ok(session.mode.lock()) = backend.mode();
        ...
        *wf_common::lock::lock_ok(session.current.lock()) = Some(shell_session.clone());
        *wf_common::lock::lock_ok(session.last_pid.lock()) = shell_session.pid();
        *wf_common::lock::lock_ok(session.status.lock()) = SessionStatus::Busy;
        *wf_common::lock::lock_ok(session.last_active_at.lock()) = wf_common::time::now();
    ...
    fn ensure_monitor_started(&self) {
        let mut monitor = wf_common::lock::lock_ok(self.monitor.lock());
        if monitor.is_some() {
            return;
        }
...
impl Drop for BackgroundShellStore {
    fn drop(&mut self) {
        ...
        // store teardown.
        let monitor = wf_common::lock::lock_ok(self.monitor.lock()).take();
