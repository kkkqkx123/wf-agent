...
pub struct LineDiff {
    pub hunks: Vec<Hunk>,
    /// Whether the resulting content ends with a trailing newline. Line
    /// tokens are stored newline-stripped, so the flag is recorded at diff
    /// time and restored by `apply_deltas` during reconstruction. `None`
    /// for empty / hand-built diffs (no content change or unknown state);
    /// reconstruction then falls back to the base content's newline state.
    #[serde(default)]
