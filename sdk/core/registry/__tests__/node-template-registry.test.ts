...
describe('NodeTemplateRegistry', () => {
  ...
  describe('register', () => {
    ...
    it('should throw if name is empty', () => {
      expect(() => registry.register(createValidStartTemplate({ name: '' }))).toThrow('is required');
    });

    it('should throw if type is invalid', () => {
      expect(() =>
        registry.register(createValidStartTemplate({ type: 'INVALID' as any })),
    ...
    it('should throw if node configuration validation fails', () => {
      expect(() =>
        registry.register(createValidLLMTemplate({ config: {} })),
      ).toThrow('Invalid node configuration');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should throw if updated config is invalid', () => {
      ...
      expect(() => registry.update('test-llm', { config: {} })).toThrow('Invalid node configuration');
    });
  });
  ...
  describe('listByCategory', () => {
    it('should filter by category', () => {
      registry.register(createValidStartTemplate({ name: 't1', metadata: { category: 'control' } }));
  ...
  describe('listByTags', () => {
    it('should filter by tags - all must match', () => {
      registry.register(createValidStartTemplate({ name: 't1', metadata: { tags: ['start', 'basic'] } }));
  ...
  describe('search', () => {
    ...
    it('should search by description', () => {
      registry.register(createValidStartTemplate({ name: 't1', description: 'custom description' }));
      expect(registry.search('custom')).toHaveLength(1);
    });

    it('should search by tags', () => {
      registry.register(createValidStartTemplate({ name: 't1', metadata: { tags: ['important', 'urgent'] } }));
      expect(registry.search('important')).toHaveLength(1);
    });

    it('should search by category', () => {
      registry.register(createValidStartTemplate({ name: 't1', metadata: { category: 'machine-learning' } }));
