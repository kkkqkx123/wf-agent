...
const STORAGE_BACKENDS: CheckpointStorageFixture[] = [
  ...
  {
    adapter: new JsonCheckpointStorage({ baseDir: tempDir }),
    backendType: "json",
    cleanup: cleanupJson,
  },
  {
    adapter: new SqliteCheckpointStorage({ dbPath: tempDb }),
    backendType: "sqlite",
    cleanup: cleanupSqlite,
  },
];
```
...
export function assertCheckpointChain(checkpoints: BaseCheckpoint[], expectedLength: number): void {
...
export function assertAgentLoopIterations(entity: AgentLoopEntity, expectedCount: number): void {
