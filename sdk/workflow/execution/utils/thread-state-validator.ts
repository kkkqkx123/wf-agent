/**
 * Thread State Transition Verification Tool Function
 *
 * Responsibilities:
 * - Define the allowed state transition rules
 * - Provide an interface for state transition verification
 * - Offer functions for querying thread states
 *
 * Design Principles:
 * - Pure functions: No side effects; the same input always produces the same output
 * - Reusability: To be shared by ThreadLifecycleCoordinator and ThreadLifecycleCoordinator
 ...
 *
 * @param threadId Thread ID (used for error messages)
 * @param currentStatus Current status
 * @param targetStatus Target status
