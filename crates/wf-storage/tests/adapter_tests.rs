...
fn make_tool(id: &str, name: &str, tool_type: wf_types::tool::ToolType) -> wf_types::tool::Tool {
    wf_types::tool::Tool {
        id: id.into(),
        name: name.into(),
        ...
        parameters: None,
        metadata: None,
        config: Some(serde_json::json!({"baseUrl": "http://example.com"})),
        enabled: Some(true),
        strict: None,
...
#[tokio::test]
async fn test_tool_definition_adapter_crud_and_list_by_type() {
    use wf_storage::adapter::tool_definition::ToolDefinitionStorageAdapter;

    let adapter = ToolDefinitionStorage::new(make_store("test_tool_definition"));
    let rest = make_tool("tool-rest-1", "fetch_data", wf_types::tool::ToolType::Rest);
    let builtin = make_tool(
        "tool-builtin-1",
        "builtin_tool",
        wf_types::tool::ToolType::BuiltIn,
    );
    adapter.save(&rest).await.unwrap();
    adapter.save(&builtin).await.unwrap();

    // Round-trip keeps the full definition (parameters/config payload).
    let loaded = adapter.load("tool-rest-1").await.unwrap().unwrap();
    assert_eq!(loaded.name, "fetch_data");
    assert_eq!(
        loaded.config,
        Some(serde_json::json!({"baseUrl": "http://example.com"}))
    );

    // Metadata filtering by tool type (ToolType serializes snake_case).
    let by_type = adapter.list_by_tool_type("rest").await.unwrap();
    assert_eq!(by_type.len(), 1);
    assert_eq!(by_type[0].id, "tool-rest-1");

    // List options (limit).
    let list = adapter
        .list(Some(
            wf_storage::adapter::tool_definition::ToolDefinitionListOptions {
                limit: Some(1),
                ..Default::default()
            },
        ))
    ...
    // Overwrite keeps a single record per id.
    let mut updated = rest.clone();
    updated.description = "updated".into();
    adapter.save(&updated).await.unwrap();
    let list_all = adapter.list(None).await.unwrap();
    assert_eq!(list_all.len(), 2);

    assert!(adapter.delete("tool-builtin-1").await.unwrap());
    assert!(!adapter.exists("tool-builtin-1").await.unwrap());
}
