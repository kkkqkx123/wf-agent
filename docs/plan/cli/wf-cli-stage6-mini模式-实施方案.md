...
| `DomainAdapter`（bootstrap/api_context/shutdown） | `domain.rs` | Stage 1 | mini 会话持有 adapter，`agent_execution::stream` 驱动 |
...
| `agent_execution::stream` + `RunAgentLoopParams`（approval_handler/agent_loop_id） | wf-api | 既有 | mini 会话启动 |
...
- [ ] `demo.rs` 最小版：`DemoSource` 合成 `UnifiedEvent` 序列（markdown 流、工具 start/end、completed）；`/fmt <kind>`、`/permission`、`/question`、`/help` 演示命令；`--demo` 走与真实会话相同的 reducer/footer 管线（不 bootstrap runtime 或 mock）。
- [ ] 勾选总方案 Stage 6 任务项；补完成记录；生成 patch（排除 target/，Cargo.lock 不动）。
- [ ] 集成测试：`tests/` 增 mini 冒烟（PTY 脚本或 `--demo` 断言）；`cargo test -p wf-cli` 全绿（Stage 2-5 无回归）。

**验收**：`wf --mini --demo` 全功能冒烟（视图切换、审批/问题、退出干净）；`cargo test -p wf-cli` 全绿；patch 校验 `grep -c 'target/'` = 0。
