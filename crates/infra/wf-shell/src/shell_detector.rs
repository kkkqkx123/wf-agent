...
//! shell-specific executable + command flag for command execution.
//!
...
impl ShellDetector {
    ...
    pub fn add_path_override(&mut self, shell_type: ShellType, path: impl Into<String>) {
        ...
        wf_common::lock::lock_ok(self.cache.lock()).remove(&shell_type);
    }
    ...
    pub fn resolve_shell_path(&self, shell_type: ShellType) -> Option<String> {
        if let Some(cached) = wf_common::lock::lock_ok(self.cache.lock()).get(&shell_type) {
            return cached.clone();
        }
