...
impl ArgumentResolver {
    ...
    ) -> ExecutionSharedResult<HashMap<String, Value>> {
    ...
    fn validate_argument(arg: &ScriptArgument, value: &Value) -> ExecutionSharedResult<()> {
...
mod tests {
    ...
    fn test_argument_resolve_default() {
        let args = vec![ScriptArgument {
            key: "port".to_string(),
            r#type: Some(crate::script::ScriptArgumentType::Number),
            required: None,
