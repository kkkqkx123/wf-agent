...
pub struct WorkflowCheckpointCoordinator {
    ...
    /// TS `contentConfig.async`: defer post-persist side effects to the
    /// background persistence queue.
    async_persistence: bool,
    /// Background persistence queue (TS `persistenceQueue`); drained by
...
impl WorkflowCheckpointCoordinator {
    ...
    /// Enable async (non-blocking) checkpoint creation (TS `contentConfig.async`):
    ...
    async fn restore_child_hierarchy(
        ...
        // Bounded concurrency for the per-child restore phase (aligned with
        // the TS Semaphore(5) child restorer).
...
impl CheckpointCoordinator for WorkflowCheckpointCoordinator {
    ...
    /// persistence queue, aligned with TS async mode.
    ...
    async fn create_checkpoint_with_strategy(
        ...
        if let Some(cadence) = self.cadence.get(&trigger) {
            if *cadence > 1 {
                let attempt = {
                    let mut entry = self.cadence_attempts.entry(entity_id.to_string()).or_insert(0);
                    *entry += 1;
                    *entry
                };
...
mod tests {
    ...
    async fn async_persistence_defers_file_snapshot_until_wait() {
        ...
        let id = coord
            .create_checkpoint(CheckpointTrigger::AfterExecute, "exec-1", make_snapshot())
            .await
            .unwrap();
        ...
        assert!(loaded.is_some(), "checkpoint itself persisted synchronously");
    }
    ...
    async fn build_writes_metadata_with_trigger_and_chain_position() {
        let coord = make_coordinator();
        let ctx = coord
            .prepare("exec-1", CheckpointTrigger::OnError)
            .await
            .unwrap();
        let cp = coord.build(ctx, make_snapshot()).await.unwrap();
    ...
    async fn create_checkpoint_with_strategy_persists_saved_id() {
        ...
        let created = coord
            ...
            .unwrap()
            .unwrap();
        assert!(coord.state_manager().load(&created).await.unwrap().is_some());
    }
