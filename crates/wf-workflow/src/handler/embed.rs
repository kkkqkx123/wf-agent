...
impl NodeHandler for EmbedHandler {
    ...
    async fn execute(&self, ctx: &mut NodeExecutionContext) -> WorkflowResult<NodeExecutionResult> {
        ...
        let graph_value = config.get("graph")
        ...
        let subgraph: WorkflowGraphStructure = match graph_value {
            Some(v) => serde_json::from_value(v.clone())
                .map_err(|e| WorkflowError::SubgraphError(format!("Invalid embed graph definition: {}", e)))?,
            None => return Err(WorkflowError::SubgraphError(
        ...
        emit_embed_event(event_bus.as_ref(), EventType::SubgraphStarted, &ctx.execution_id, &ctx.node_id);
        ...
        emit_embed_event(event_bus.as_ref(), EventType::SubgraphCompleted, &ctx.execution_id, &ctx.node_id);
...
fn emit_embed_event(event_bus: Option<&Arc<wf_core::EventBus>>, event_type: EventType, execution_id: &wf_types::Id, node_id: &str) {
    ...
    let event = BaseEvent {
        ...
        metadata: Some(HashMap::from([
            ...
            ("node_type".to_string(), Value::String("EMBED_GRAPH".to_string())),
        ])),
    };
